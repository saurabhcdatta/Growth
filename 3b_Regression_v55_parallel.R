############################################################
# PART 3b v5.0 — PARALLEL VERSION — FCU & FISCU ASSET FORECASTING
#
...
# ── Parallel config ──────────────────────────────────────────
# Use all but 1 core (leave one for OS responsiveness)
...
# ── Build task list: one task per (dv, cat) combination ──────
task_list <- list()
...
# ── Worker function: runs all rolling windows for one (dv, cat) ──
run_one_model_task <- function(task, qtrly, DEP_VARS, FEATS_ALL,
  ...
  for (tq in test_quarters) {
    ...
    # TSCV model fit
    res <- tryCatch(
      fit_window_3b(train_dt, test_row, dv, feats_dv,
    ...
    # Pure ARIMA 1-step OOS benchmark
...
clusterExport(cl, varlist = c(
  ...
  "fit_window_3b", "prep_X", "build_coef_dt", "winsorise", "reg_metrics",
  "run_one_model_task"
), envir = environment())

# Run all (dv, cat) combinations in parallel
par_results <- tryCatch(
  parLapply(cl, task_list, function(task) {
    run_one_model_task(
      task, qtrly, DEP_VARS, FEATS_ALL,
      EXIT_VARS, EXIT_VARS_RAW, EXIT_SIGN_PRIOR,
      SPARSE_CATS, TSCV_MIN_TRAIN, TSCV_MIN_TRAIN_SPARSE,
      test_quarters, MAX_XREG_VARS, SIG_LEVEL, TSCV_H, YOY_CAP
    )
...
for (r in par_results) {
  ...
  if (!is.null(r$m_met)) {
    ...
    message(sprintf("    [%s] RMSE=%.3f  R²=%.3f  n=%d",
                    key, r$m_met$rmse, r$m_met$r2_oos, r$m_met$n))
  }
  if (!is.null(r$fit_info))     all_fits[[key]]       <- r$fit_info
}
# Flatten arima_oos list into proper list of data.tables
