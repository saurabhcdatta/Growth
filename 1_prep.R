...
## Sample starts 2005Q1 (data considered reliable from there). Regions 1, 2, 3.
...
panel <- raw %>%
  filter(cu_type %in% c(1, 2),
         ...
         year >= 2005, year <= 2026,
         !(year == 2026 & quarter > 1)) %>%
  mutate(q_index   = (year - 2005) * 4 + quarter,
...
## ---------------------------------------------------------------------
Q_MIN <- 1
Q_MAX <- max(panel$q_index)          # 2026q1 -> 85
N_Q   <- Q_MAX - Q_MIN + 1
...
for (i in seq_len(nrow(cells))) {
  ...
  ts_list[[i]] <- ts(y_i, start = c(2005, 1), frequency = 4)
}
...
## Region 8 (ONES) is excluded. Some region 1/2/3 x $10B+ cells are still thin.
## ---------------------------------------------------------------------
MIN_TRAIN <- 32                      # quarters (8 years) before the first CV origin
