...
# Also drop anything with extreme correlation with the dependent variable
# (a final safety net catching sub-components we may have missed)
TAUTOLOGY_CORR_THRESHOLD <- 0.95
target_for_check <- "yoy_fcu_assets_pct"
high_corr_vars <- character(0)
if (target_for_check %in% num_cols) {
  y <- panel[[target_for_check]]
  ...
  for (v in candidates) {
    ...
    if (!is.na(r) && abs(r) >= TAUTOLOGY_CORR_THRESHOLD) {
...
message(sprintf("    High-corr (r>%.2f): %d excluded (mechanical sub-components)",
...
if (length(high_corr_vars) > 0) {
  message(sprintf("\n  Variables with |corr| >= %.2f to %s removed:",
                  TAUTOLOGY_CORR_THRESHOLD, target_for_check))
