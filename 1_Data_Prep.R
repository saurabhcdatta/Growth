...
qtrly[, paste0("yoy_", fe_cols) :=
        lapply(.SD, function(x) {
          lag4 <- shift(x, 4L, type="lag")
          pct_chg(x, lag4)
        }),
      .SDcols = fe_cols,
      by = categories]
...
qtrly[, paste0("qoq_", fe_cols) :=
        lapply(.SD, function(x) {
          lag1 <- shift(x, 1L, type="lag")
          pct_chg(x, lag1)
        }),
      .SDcols = fe_cols,
      by = categories]
...
for (lag_n in c(1L, 2L, 4L, 8L)) {
  new_nms <- paste0(key_lag_vars, "_lag", lag_n)
  qtrly[, (new_nms) :=
          lapply(.SD, function(x) shift(x, lag_n, type="lag")),
        .SDcols = key_lag_vars,
        by = categories]
}
toc()
...
qtrly[, paste0("yoy_", ma_rate_vars) :=
        lapply(.SD, function(x) pct_chg(x, shift(x, 4L))),
      .SDcols = ma_rate_vars,
      by = categories]

qtrly[, paste0("qoq_", ma_rate_vars) :=
...
for (v in share_vars) {
  ...
  qtrly[, (paste0("yoy_", sh_col)) :=
          pct_chg(get(sh_col), shift(get(sh_col), 4L)),
        by=categories]
...
qtrly[, `:=`(
  ...
  # PRIMARY dep var: YoY % change in ficu_count
  # (stationary, used in rolling-window XGBoost)
  yoy_ficu_pct = pct_chg(ficu_count, shift(ficu_count, 4L)),

  # SECONDARY: QoQ % change (for short-run models)
  qoq_ficu_pct = pct_chg(ficu_count, shift(ficu_count, 1L)),

  # TERTIARY: log-difference in total CUs across all categories
  # (system-level alternative)
  ld_ficu = log(pmax(ficu_count,1)) - log(pmax(shift(ficu_count,4L),1)),

  # Net entry rate: (entrants - exits) / beginning stock
  # Requires n_active vs n_total — proxy:
  net_entry_rate = fifelse(
    shift(ficu_count, 1L, type="lag") > 0,
    (ficu_count - shift(ficu_count, 1L)) /
      shift(ficu_count, 1L) * 100,
    NA_real_
  ),

  # 4-quarter ahead dep var (for forecasting horizon clarity)
  ficu_count_lag4 = shift(ficu_count, 4L, type="lag")
