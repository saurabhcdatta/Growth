############################################################
# PART 1 v4.0 — PANEL ARIMA PROJECTION + FEATURE ENGINEERING
#              (FRB-Compatible)
#
# Architecture vs v3:
#   Identical ARIMA forecasting of every panel series.
#   Changes are minimal — only what is needed to align with
#   the v4 macro pipeline (macro_v4_frb.R):
#     • PLOT_DIR       → plots_v4/  (shared with macro_v4_frb.R)
#     • HIST_END       → explicit config matching macro_v4_frb.R
#     • NTFY_ENABLED   → FALSE by default (script is fast enough)
#     • save_plot()    → pdf() device, no cairo_pdf dependency
#     • yoy_assets_pct → added to Section 7 dep vars
#     • Output names   → qtrly_forecast_v4.rds,
#                        forecast_horizon_summary_v4.csv
#     • Next-step msg  → macro_v4_frb.R (not macro_v3_arima.R)
#
# Unchanged from v3:
#   All ARIMA fitting (Section 4), all feature engineering
#   (Sections 6a-6k), aggregation, combine, QC, plots P1-P11.
#   Output files qtrly_enriched_v3.rds and qtrly_full_v3.rds
#   keep their _v3 suffix so macro_v4_frb.R reads them unchanged.
#
# Outputs:
#   qtrly_enriched_v3.rds          historical panel
#   qtrly_forecast_v4.rds          forecast quarters only
#   qtrly_full_v3.rds              historical + forecast (full)
#   arima_diagnostics_v3.csv       model order + AICc per series
#   arima_models_v3.rds            fitted model objects
#   forecast_horizon_summary_v4.csv point forecast at 1/3/5yr
#   11 diagnostic PDF plots → plots_v4/
#
...
# Last quarter of actual data — must match macro_v4_frb.R HIST_END
...
PLOT_DIR <- file.path(DATA_DIR, "plots_v4")   # shared with macro_v4_frb.R
...
# Per-CU-type asset columns (used to build yoy_ficu_assets_pct / yoy_fiscu_assets_pct)
cr[, `:=`(
  ficu_assets_tot  = fifelse(cu_type == 1, assets_tot, 0),
...
detect_agg_type <- function(dt, cols) {
  ...
  level_pat <- c("^acct_","^lns_","^ins_","^inv_","^dep_","^exp_",
                 ...
                 "ficu_count$","fiscu_count$","ficu_assets$","fiscu_assets$",
...
qtrly_counts <- cr_a[, .(
  ...
  ficu_assets   = sum(ficu_assets_tot,  na.rm=TRUE),
...
ci_vars      <- c("ficu_count","fiscu_count","ficu_assets","fiscu_assets",
                  "assets_tot","members",
...
share_vars <- intersect(c("ficu_count","fiscu_count","ficu_assets","fiscu_assets",
...
qtrly_full[, `:=`(
  ...
  # FICU Assets
  yoy_ficu_assets_pct  = pct_chg(ficu_assets, shift(ficu_assets,4L)),
  qoq_ficu_assets_pct  = pct_chg(ficu_assets, shift(ficu_assets,1L)),
  ficu_assets_lag4     = shift(ficu_assets,4L),
  # FISCU Assets
...
saveRDS(qtrly_fc, "qtrly_forecast_v4.rds")
message(sprintf("    qtrly_forecast_v4.rds  — %d × %d  (forecast)",
...
horizon_sum <- qtrly_full[is_forecast==TRUE &
                           ...
                           .(cat_label, date,
                             ...
                             yoy_ficu_assets_pct, yoy_fiscu_assets_pct,
                             ln_assets_tot,
...
fwrite(horizon_sum, "forecast_horizon_summary_v4.csv")
message("    forecast_horizon_summary_v4.csv")
toc()
...
message("\n── Next step: run macro_v4_frb.R to merge FRB macro features ──")
message("   Then run part3_regression_pipeline.R for modeling")
