...
share_vars <- c("ficu_count","fiscu_count","assets_tot","members",
                "lns_tot","dep_tot")
...
qtrly[, `:=`(
  ...
  # ── FICU (federally insured CU) dep vars ───────────────
  ...
  ld_ficu         = log(pmax(ficu_count, 1L)) -
  ...
  # ── FISCU (federally insured state CU) dep vars ────────
  yoy_fiscu_pct    = pct_chg(fiscu_count, data.table::shift(fiscu_count, n=4L)),
  qoq_fiscu_pct    = pct_chg(fiscu_count, data.table::shift(fiscu_count, n=1L)),
  ld_fiscu         = log(pmax(fiscu_count, 1L)) -
                     log(pmax(data.table::shift(fiscu_count, n=4L), 1L)),
  net_entry_rate_fiscu = fifelse(
    data.table::shift(fiscu_count, n=1L) > 0,
    (fiscu_count - data.table::shift(fiscu_count, n=1L)) /
      data.table::shift(fiscu_count, n=1L) * 100,
    NA_real_
  ),
  fiscu_count_lag4 = data.table::shift(fiscu_count, n=4L)

), by = categories]

message(sprintf("    yoy_ficu_pct  range: [%.2f%%, %.2f%%]  non-NA: %d / %d",
                min(qtrly$yoy_ficu_pct,  na.rm=TRUE),
                max(qtrly$yoy_ficu_pct,  na.rm=TRUE),
                sum(!is.na(qtrly$yoy_ficu_pct)),  nrow(qtrly)))
message(sprintf("    yoy_fiscu_pct range: [%.2f%%, %.2f%%]  non-NA: %d / %d",
                min(qtrly$yoy_fiscu_pct, na.rm=TRUE),
                max(qtrly$yoy_fiscu_pct, na.rm=TRUE),
                sum(!is.na(qtrly$yoy_fiscu_pct)), nrow(qtrly)))
toc()
...
key_cols <- c("date","categories","cat_label",
              "ficu_count","fiscu_count",
              "yoy_ficu_pct","qoq_ficu_pct","net_entry_rate","ficu_count_lag4","ld_ficu",
              "yoy_fiscu_pct","qoq_fiscu_pct","net_entry_rate_fiscu","fiscu_count_lag4","ld_fiscu",
              "n_mergers","n_liquid","n_acquisition",
...
message(sprintf("          yoy_fiscu_pct qoq_fiscu_pct net_entry_rate_fiscu ld_fiscu"))
message("=======================================================")

# Quick preview of dep vars by category
message("\nFICU dep var summary by category:")
...
message("\nFISCU dep var summary by category:")
print(qtrly_save[!is.na(yoy_fiscu_pct),
                  .(n     = .N,
                    mean  = round(mean(yoy_fiscu_pct),  2),
                    sd    = round(sd(yoy_fiscu_pct),    2),
                    min   = round(min(yoy_fiscu_pct),   2),
                    max   = round(max(yoy_fiscu_pct),   2)),
                  by=cat_label][order(cat_label)])
