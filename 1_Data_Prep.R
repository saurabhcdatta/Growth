...
suppressPackageStartupMessages({
  library(haven);      library(data.table); library(zoo)
  library(lubridate);  library(forecast);   library(httr)
...
cr[, `:=`(
  ...
  ln_assets   = log(pmax(assets_tot, 1)),
  categories  = fifelse(assets_cat2 %in% c(6L,7L), 6L,
                  fifelse(assets_cat2 == 8L, 7L,
                          as.integer(assets_cat2))),
  date = zoo::as.yearqtr(
    make_date(cryear, (crquarter - 1L)*3L + 1L, 1L)),
...
fc_template[, categories := as.integer(sub(".*_(\\d+).*","\\1",
                               match(cat_label,cats)))]
# Safer: pull categories from qtrly
...
# Add any CI columns present in fc_template but missing in qtrly
ci_extra_cols <- grep("_lo80$|_hi80$|_lo95$|_hi95$|is_forecast|horizon_q|is_1yr|is_3yr|is_5yr",
                      names(fc_template), value=TRUE)
for (cn in ci_extra_cols) {
  if (!cn %in% names(qtrly)) qtrly[, (cn) := NA]
}

# Align: only keep cols present in both + CI cols
shared <- union(names(qtrly), names(fc_template))
for (cn in setdiff(shared, names(qtrly)))     qtrly[,       (cn) := NA]
for (cn in setdiff(shared, names(fc_template))) fc_template[, (cn) := NA]

qtrly_full <- rbindlist(list(qtrly, fc_template), fill=TRUE)
