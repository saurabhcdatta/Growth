############################################################
# PART 3a v5.0 — PARALLEL VERSION — FCU & FISCU COUNT FORECASTING
#
# Targets  : yoy_fcu_pct   (YoY % change in FCU count)
...
message(sprintf("    Parallel cores: %d / %d available", N_CORES, detectCores()))
...
required <- c("date","categories","cat_label","yoy_fcu_pct","yoy_fiscu_pct")
...
DEP_VARS <- list(
  yoy_fcu_pct  = list(label="FCU YoY % Change",  short="fcu"),
...
HARD_EXCL <- c("yoy_fcu_pct","yoy_fiscu_pct","yoy_assets_pct",
               "qoq_fcu_pct","qoq_fiscu_pct",
               "yoy_fcu_count","yoy_fiscu_count",
               "qoq_fcu_count","qoq_fiscu_count",
...
for (dv in names(DEP_VARS)) {
  for (cat in cats) {
    task_list[[length(task_list) + 1L]] <- list(dv = dv, cat = cat)
  }
}
n_tasks <- length(task_list)
message(sprintf("    Parallel task grid: %d models across %d cores", n_tasks, N_CORES))

# ── Worker function ──────────────────────────────────────────
run_one_model_task <- function(task, qtrly, DEP_VARS, FEATS_ALL,
                                EXIT_VARS, EXIT_VARS_RAW, EXIT_SIGN_PRIOR,
                                SPARSE_CATS, TSCV_MIN_TRAIN, TSCV_MIN_TRAIN_SPARSE,
                                test_quarters, MAX_XREG_VARS, SIG_LEVEL,
                                TSCV_H, YOY_CAP) {
  suppressPackageStartupMessages({
    library(data.table); library(zoo); library(glmnet); library(forecast)
  })
  `%||%` <- function(a, b) if (!is.null(a) && length(a) > 0 && !is.na(a[1])) a else b

  dv <- task$dv; cat <- task$cat
  dv_label <- DEP_VARS[[dv]]$label; dv_short <- DEP_VARS[[dv]]$short

  cat_dt <- qtrly[cat_label == cat]; setorderv(cat_dt, "date")
  fc_rows <- list(); coef_rows <- list(); arima_oos_rows <- list(); last_res <- NULL
  ...
  for (tq in test_quarters) {
    train_idx <- cat_dt$date >= zoo::as.yearqtr("2005 Q1") & cat_dt$date < tq
    test_idx  <- cat_dt$date == tq
    if (sum(train_idx) < min_train_cat || sum(test_idx) == 0) next
    train_dt <- cat_dt[train_idx]; test_row <- cat_dt[test_idx][1L]

    res <- tryCatch(
      fit_window_3a(train_dt, test_row, dv, feats_dv,
                    MAX_XREG_VARS, TSCV_H, SIG_LEVEL,
                    EXIT_VARS, EXIT_SIGN_PRIOR, YOY_CAP,
    ...
    arima_oos_pred <- tryCatch({
      ...
      fit_b  <- forecast::auto.arima(y_ts_b, stepwise=TRUE, approximation=TRUE,
                                     max.p=3L, max.q=2L, max.P=1L, max.Q=1L)
      fc_b   <- forecast::forecast(fit_b, h=1L, level=95L)
      list(pred=as.numeric(fc_b$mean), lo95=as.numeric(fc_b$lower[1L]),
           hi95=as.numeric(fc_b$upper[1L]),
           order=paste0("ARIMA(", paste(forecast::arimaorder(fit_b), collapse=","), ")"))
    }, error = function(e) NULL)

    actual_val <- test_row[[dv]]
    if (is.na(actual_val)) {
      col_lv <- if (dv=="yoy_fcu_pct") "fcu_count" else "fiscu_count"
    ...
    if (!res$ok) {
      fc_rows[[length(fc_rows)+1L]] <- data.table(
        dep_var=dv, dv_label=dv_label, cat_label=cat, date=tq, actual=actual_val,
        pred_final=NA_real_, pred_lo95=NA_real_, pred_hi95=NA_real_,
        method_used="FAILED", error_msg=res$reason,
        adj_r2=NA_real_, tscv_rmse=NA_real_, n_lasso_sel=NA_integer_, n_final=NA_integer_)
      next
    }

    fc_rows[[length(fc_rows)+1L]] <- data.table(
      dep_var=dv, dv_label=dv_label, cat_label=cat, date=tq, actual=actual_val,
      pred_final=res$pred_final, pred_lo95=res$pred_lo95, pred_hi95=res$pred_hi95,
      method_used=res$method_used, error_msg=NA_character_,
      adj_r2=res$adj_r2, tscv_rmse=res$tscv_rmse,
      n_lasso_sel=as.integer(res$n_lasso_sel), n_final=as.integer(res$n_final))

    per_win_cd <- if (!is.null(res$arimax_fit)) {
      build_coef_dt(res$arimax_fit, res$n_train %||% 20L, res$sig_vars %||% character(0))
    } else { res$coef_dt }
    if (!is.null(per_win_cd) && nrow(per_win_cd) > 0) {
      cd_w <- copy(per_win_cd); cd_w[, `:=`(dep_var=dv, cat_label=cat, date=tq)]
      coef_rows[[length(coef_rows)+1L]] <- cd_w
    }
    if (!is.null(arima_oos_pred)) {
      arima_oos_rows[[length(arima_oos_rows)+1L]] <- data.table(
        dep_var=dv, cat_label=cat, date=tq, actual=actual_val,
        arima_pred=arima_oos_pred$pred, arima_lo95=arima_oos_pred$lo95,
        arima_hi95=arima_oos_pred$hi95, arima_order=arima_oos_pred$order)
    }
  }

  fc_dt <- if (length(fc_rows) > 0) rbindlist(fc_rows, fill=TRUE) else data.table()
  coef_dt <- if (length(coef_rows) > 0) rbindlist(coef_rows, fill=TRUE) else data.table()
  arima_oos_dt <- if (length(arima_oos_rows) > 0) rbindlist(arima_oos_rows, fill=TRUE) else data.table()
  ...
  if (nrow(fc_dt) > 0) {
    ...
    if (nrow(valid) >= 2L) {
      ...
      m_met$dep_var <- dv; m_met$dv_label <- dv_label; m_met$cat_label <- cat
    }
  }

  fit_info <- NULL
  if (!is.null(last_res) && isTRUE(last_res$ok)) {
    fit_info <- list(fit=last_res$arimax_fit, sig_vars=last_res$sig_vars,
                     dep_var=dv, cat_label=cat, n_train=last_res$n_train,
                     date_end=max(test_quarters))
  }

  list(key=paste(dv, cat, sep="|"), dv=dv, cat=cat, fc_dt=fc_dt,
       coef_dt=coef_dt, arima_oos_dt=arima_oos_dt, m_met=m_met, fit_info=fit_info)
}

# ── Launch parallel cluster ──────────────────────────────────
message(sprintf("\n    Launching %d-core cluster...", N_CORES))
tic("Parallel TSCV")
cl <- makeCluster(N_CORES)
clusterExport(cl, varlist = c(
  "qtrly", "DEP_VARS", "FEATS_ALL",
  "EXIT_VARS", "EXIT_VARS_RAW", "EXIT_SIGN_PRIOR",
  "SPARSE_CATS", "TSCV_MIN_TRAIN", "TSCV_MIN_TRAIN_SPARSE",
  "test_quarters", "MAX_XREG_VARS", "SIG_LEVEL", "TSCV_H", "YOY_CAP",
  "fit_window_3a", "prep_X", "build_coef_dt", "winsorise", "reg_metrics",
  "run_one_model_task"
), envir = environment())

par_results <- tryCatch(
  parLapply(cl, task_list, function(task) {
    run_one_model_task(task, qtrly, DEP_VARS, FEATS_ALL,
      EXIT_VARS, EXIT_VARS_RAW, EXIT_SIGN_PRIOR,
      SPARSE_CATS, TSCV_MIN_TRAIN, TSCV_MIN_TRAIN_SPARSE,
      test_quarters, MAX_XREG_VARS, SIG_LEVEL, TSCV_H, YOY_CAP)
  }),
  ...
  finally = { stopCluster(cl); message("    Cluster stopped.") }
)
toc()

# ── Unpack results ───────────────────────────────────────────
message(sprintf("    Unpacking %d model results...", length(par_results)))
for (r in par_results) {
  key <- r$key
  if (nrow(r$fc_dt) > 0)        all_forecasts[[key]] <- r$fc_dt
  if (nrow(r$coef_dt) > 0)      all_coefs[[key]]     <- r$coef_dt
  if (nrow(r$arima_oos_dt) > 0)  all_arima_oos[[key]] <- r$arima_oos_dt
  if (!is.null(r$m_met)) {
    all_metrics[[key]] <- r$m_met
    message(sprintf("    [%s] RMSE=%.3f  R²=%.3f  n=%d", key, r$m_met$rmse, r$m_met$r2_oos, r$m_met$n))
  }
  if (!is.null(r$fit_info)) all_fits[[key]] <- r$fit_info
}
all_arima_oos <- unlist(all_arima_oos, recursive=FALSE)
if (!is.list(all_arima_oos)) all_arima_oos <- list()
message(sprintf("    Parallel TSCV complete: %d models fitted", length(par_results)))
...
fwrite(forecasts_all, file.path(RESULT_DIR, "forecasts_3a_v5.csv"))
fwrite(metrics_all,   file.path(RESULT_DIR, "metrics_3a_v5.csv"))
fwrite(coefs_all,     file.path(RESULT_DIR, "coefficients_3a_v5.csv"))
...
if (nrow(arima_oos_all)>0)
  fwrite(arima_oos_all, file.path(RESULT_DIR, "arima_oos_benchmark_3a_v5.csv"))
...
full_levels <- qtrly[, .(cat_label, date, fcu_count, fiscu_count)]
...
level_fcu  <- make_level_fc("yoy_fcu_pct",  "fcu_count")
level_fiscu <- make_level_fc("yoy_fiscu_pct", "fiscu_count")

level_fc <- rbindlist(list(level_fcu, level_fiscu), fill=TRUE)
if (nrow(level_fc) == 0L) {
  level_fc <- data.table(cat_label=character(), date=numeric(),
    pred_yoy=numeric(), lo95_yoy=numeric(), hi95_yoy=numeric(),
    pred_level=numeric(), lo95_level=numeric(), hi95_level=numeric(),
    actual_level=numeric(), series=character(), method_used=character())
}
...
fwrite(level_fc,      file.path(RESULT_DIR, "level_forecasts_3a_v5.csv"))
fwrite(level_metrics, file.path(RESULT_DIR, "level_metrics_3a_v5.csv"))
...
for (key in names(all_fits)) {
  ...
  # Level anchors
  count_col  <- if (dv=="yoy_fcu_pct") "fcu_count" else "fiscu_count"
...
for (ser in c("fcu_count","fiscu_count")) {
...
for (ser in c("fcu_count","fiscu_count")) {
  dv_ser  <- if (ser=="fcu_count") "yoy_fcu_pct" else "yoy_fiscu_pct"
...
# P9: FCU policy chart
if ("fcu_count" %in% names(qtrly)) {
  message("    P9: FCU Count policy chart...")
  make_policy_chart_3a(
    count_col  = "fcu_count",
    title_text = "FCU Count — Historical Trend & ARIMAX Forecast by Asset Category",
    stem       = "P9_policy_fcu_count.pdf"
  )
} else {
  message("    P9: skipped - fcu_count not in data")
}
...
# ── Series metadata ───────────────────────────────────────────
series_meta_3a <- list(
  fcu_count  = list(label = "FCU Count",  hist_col = "fcu_count",
                     short = "fcu",  dep_var = "yoy_fcu_pct"),
  fiscu_count = list(label = "FISCU Count", hist_col = "fiscu_count",
                     short = "fiscu", dep_var = "yoy_fiscu_pct")
)
  ...
  for (cat in cats) {
    ...
    if (needs_refit) {
      ...
      if (!is.null(fit_new) && !is.null(fc_new)) {
        ...
        message(sprintf("      [TREND GUARD] ACCEPTED: forecast range %.0f–%.0f",
                        min(new_vals), max(new_vals)))
      } else {
        message(sprintf("      [TREND GUARD] WARNING: all refits failed for %s | %s",
                        sr, cat))
      }
    }
...
for (sr in names(series_meta_3a)) {
  ...
  hm_pnum <- if (sr == "fcu_count") "P12a" else "P12b"
...
for (sr in names(series_meta_3a)) {
  ...
  p11ab_stem <- if (sr == "fcu_count") "P11A" else "P11B"
...
if (nrow(arima_base_all) == 0) {
  ...
  for (sr in names(series_meta_3a)) {
    ...
    hm_pnum_mean <- if (sr == "fcu_count") "P16A" else "P16B"
    ...
    hm_pnum_ci <- if (sr == "fcu_count") "P16AA" else "P16BB"
...
message("\n[12] Done. All outputs in results_3a/ and plots_3a/")
