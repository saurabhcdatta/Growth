...
# ── 2c-bis. ALLOW-LIST FILTER ────────────────────────────
# Only keep macro variables that are in the FRB Variable Data
# Dictionary. This excludes structural CCAR scenario variables
# (depreciation rates, capital stocks, time trends) that rank
# spuriously via trend correlation but have no causal link to
# CU dynamics.
#
# The allow-list is the union of:
#   - VAR_MAP sheet (raw FRB short names)
#   - Derived_Series sheet (composite indicators)
#
# Set ALLOW_LIST_FILTER <- FALSE to disable (run on full pool).
ALLOW_LIST_FILTER <- TRUE

if (ALLOW_LIST_FILTER) {

  # Curated allow-list of base names (lowercase) from the dictionary.
  # If new variables are added to the dictionary, update this list.
  ALLOWED_BASE_NAMES <- c(
    # ── RATES & MONETARY POLICY ──
    "fedfunds", "fedfunds_eff", "fedfunds_rule",
    "gs3m", "gs3", "gs7", "gs10", "gs10_6m", "gs30",
    "gs1", "gs3_cmt", "gs7_cmt", "gs10_cmt", "gs20_cmt", "gs30_cmt",
    "mortgage30", "mortgage15", "prime_rate",
    "mpe2y", "mpe10y", "mpe310y",
    "tbill3m_fed", "gs10_fed", "gs5_fed", "sofr", "gs1m_cmt",

    # ── SPREADS ──
    "on_off_10yr", "on_off_5yr",
    "baa_spread", "bbb10_spread", "bbb10_yield",
    "mtg_spread", "prime_spread", "discount_spread",
    "ior_spread", "sofr_spread",

    # ── FORWARD RATES ──
    "fwd_1y1y", "fwd_1y2y", "fwd_1y3y", "fwd_1y4y", "fwd_1y5y",
    "fwd_1y6y", "fwd_1y7y", "fwd_1y8y", "fwd_1y9y",
    "fwd_5y1y", "fwd_5y2y", "fwd_5y3y", "fwd_5y4y", "fwd_5y5y",

    # ── LABOR MARKET ──
    "unrate", "nairu", "lfpr", "lfpr_demo",
    "unemp_duration", "avg_ui_benefit", "initial_claims",

    # ── OUTPUT & ACTIVITY ──
    "gdp_real", "indpro", "indpro_mfg", "cap_stock",
    "pce_total", "pce_durable", "pce_mvp",
    "pce_nondurable", "pce_food", "pce_gasoline",
    "consumption", "consumption_mvp", "pce_services",
    "fixed_invest",

    # ── PRICES & INFLATION ──
    "cpi", "core_cpi", "ppi", "oil_brent",
    "pce_deflator_nrg", "hpi_fed", "infl_target", "div_tax_wedge",

    # ── HOUSING ──
    "housing_expect", "housing_permits",
    "cs20_hpi", "fhfa_hpi", "fhfa_purchase_hpi",
    "rent_sfh", "rent_mfh", "rent_nonres",
    "foreclosure_inv", "foreclosure_start",
    "new_home_price", "cre_price",

    # ── FINANCIAL MARKETS ──
    "sp500_mktcap", "djia_total", "djia", "nasdaq", "msci_sc",

    # ── CREDIT QUALITY ──
    "cc_delinq", "cc_chargeoff",
    "cons_loan_delinq", "cons_loan_chargeoff",
    "ci_loan_delinq", "ci_loan_chargeoff",
    "corp_leverage", "consumer_bankrupt",

    # ── CONSUMER CREDIT & DEBT ──
    "revolving_credit", "nonrev_credit",
    "total_credit", "mortgage_debt",

    # ── HOUSEHOLD FINANCE ──
    "disp_income", "labor_income", "interest_pymts",
    "personal_int_inc", "dividend_inc", "net_int_misc",
    "savings_rate", "undist_profits",

    # ── SENTIMENT ──
    "cons_confidence",

    # ── DERIVED SERIES (Sheet 2 of dictionary) ──
    "yield_curve", "spread_2s10s",
    "yield_curve_inv", "yield_curve_inv_run",
    "cpi_yoy", "core_cpi_yoy",
    "real_rate", "hy_spread", "credit_tightness",
    "fedfunds_trail8", "fedfunds_cycle",
    "fedfunds_chg", "fomc_regime", "hike_run"
  )

  # Strip transformation suffixes/prefixes to recover base name
  strip_to_base <- function(v) {
    v <- tolower(v)
    v <- gsub("^macro_", "", v)
    v <- gsub("^yoy_", "", v)
    v <- gsub("^qoq_", "", v)
  ...
  bases <- vapply(macro_feats, strip_to_base, character(1))
  in_allowlist <- bases %in% ALLOWED_BASE_NAMES

  dropped <- macro_feats[!in_allowlist]
  macro_feats <- macro_feats[in_allowlist]

  message(sprintf("  Allow-list filter applied: kept %d / %d macro variables",
                  length(macro_feats), length(macro_feats) + length(dropped)))
  if (length(dropped) > 0) {
    # Show unique dropped base names so user can verify nothing important was lost
    dropped_bases <- unique(vapply(dropped, strip_to_base, character(1)))
    message(sprintf("  Dropped %d unique base variables (not in dictionary):",
                    length(dropped_bases)))
    for (b in head(sort(dropped_bases), 30))
      message(sprintf("    - %s", b))
    if (length(dropped_bases) > 30)
      message(sprintf("    ... and %d more", length(dropped_bases) - 30))
  }
}
