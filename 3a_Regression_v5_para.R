...
TSCV_EVAL_K           <- 6L    # number of TSCV folds to evaluate (6 ≈ 70% faster than 20)
...
tscv_rmse <- function(y_ts, xreg_mat, arima_ord, has_seas,
  ...
  # For efficiency: evaluate only last TSCV_EVAL_K folds (default 6).
  # 6 recent OOS errors are sufficient for variable selection while
  # cutting runtime by ~70% vs 20 folds.
  eval_start <- max(min_train, n - TSCV_EVAL_K)
...
all_arima_oos <- list()        # populated in post-parallel collection

model_id <- 0L

# ── Build task list ────────────────────────────────────────────
model_tasks <- expand.grid(dv = names(DEP_VARS), cat = cats,
                           stringsAsFactors = FALSE)
n_models <- nrow(model_tasks)
...
clusterExport(cl, varlist = ls(envir = worker_env), envir = worker_env)
clusterEvalQ(cl, {
  suppressPackageStartupMessages({
...
message("    Workers ready. Starting parallel model fitting...")
tic("Parallel model loop")

par_results <- foreach(
  task_i = seq_len(n_models),
  .packages = c("data.table", "zoo", "glmnet", "forecast"),
  .errorhandling = "pass"
) %dopar% {

  dv  <- model_tasks$dv[task_i]
  cat <- model_tasks$cat[task_i]
  dv_label <- DEP_VARS[[dv]]$label
  dv_short <- DEP_VARS[[dv]]$short

  cat(sprintf("[Model %02d/%02d] %s | %s  -- started %s\n",
    ...
    # Return results from this parallel task
    list(
      key       = paste(dv, cat, sep="|"),
      dv        = dv,
      dv_label  = dv_label,
      cat       = cat,
      fc_dt     = if (length(fc_rows) > 0) rbindlist(fc_rows, fill=TRUE) else NULL,
      coef_rows = coef_rows,
      arima_oos = arima_oos_rows,
      fit_info  = if (!is.null(last_res) && last_res$ok) list(
...
for (res in par_results) {
  if (inherits(res, "error") || is.null(res)) {
    ...
    if (inherits(res, "error"))
      message(sprintf("    [FAIL] %s", conditionMessage(res)))
    next
  }
  n_ok <- n_ok + 1L
  key <- res$key
  if (!is.null(res$fc_dt)) {
    all_forecasts[[key]] <- res$fc_dt
    valid <- res$fc_dt[!is.na(actual) & !is.na(pred_final)]
    if (nrow(valid) >= 2L) {
      m_met <- reg_metrics(valid$actual, valid$pred_final)
      m_met$dep_var   <- res$dv
      m_met$dv_label  <- res$dv_label
      m_met$cat_label <- res$cat
      all_metrics[[key]] <- m_met
      message(sprintf("    [%02d] %s | %s  RMSE=%.3f  R2=%.3f",
                      n_ok, res$dv, res$cat, m_met$rmse, m_met$r2_oos))
    }
  }
  if (length(res$coef_rows) > 0)
    all_coefs[[key]] <- rbindlist(res$coef_rows, fill=TRUE)
  if (length(res$arima_oos) > 0)
    all_arima_oos <- c(all_arima_oos, res$arima_oos)
  if (!is.null(res$fit_info))
    all_fits[[key]] <- res$fit_info
}
message(sprintf("    Models: %d ok, %d failed (of %d)", n_ok, n_fail, n_models))

# Show errors from any failed models
if (n_fail > 0) {
  message("    --- Failed model details ---")
  for (k in seq_along(par_results)) {
    res <- par_results[[k]]
    if (inherits(res, "error")) {
      message(sprintf("    Task %d: %s", k, conditionMessage(res)))
    } else if (is.null(res)) {
      message(sprintf("    Task %d: returned NULL (insufficient data)", k))
    }
  }
}

message(sprintf("    forecasts collected: %d items, %d total rows",
                length(all_forecasts),
                sum(vapply(all_forecasts, nrow, 0L))))
...
level_fcu  <- if (nrow(forecasts_all) > 0 && "dep_var" %in% names(forecasts_all))
                make_level_fc("yoy_fcu_pct",  "fcu_count") else data.table()
level_fiscu <- if (nrow(forecasts_all) > 0 && "dep_var" %in% names(forecasts_all))
                 make_level_fc("yoy_fiscu_pct", "fiscu_count") else data.table()
