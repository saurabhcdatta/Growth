############################################################
# PART 3c v5.0 — CORPORATE CREDIT UNION ASSET FORECASTING
#
# Target   : yoy_corp_cu_assets_pct (YoY % change in
#            Corporate CU total assets)
#            Single aggregate series → 1 model
#
# Theory   : Corporate CU asset growth responds to macro-
#            economic conditions (interest rates, credit
#            spreads, unemployment, GDP growth, etc.).
#            Unlike natural-person CUs, corporate CUs do
#            NOT have merger/liquidation exit dynamics, so
#            exit variables are excluded.
#
# Features : FRB Baseline 2026 macro variables only
#            Seasonal dummies q1/q2/q3
#            NO exit variables (corporate CUs)
#
# Method   : Same as 3a/3b:
#            1. auto.arima() on training series → ARIMA order
#            2. TSCV with expanding window for xreg selection
#            3. Final model fit on full training window
#            4. Multi-step ARIMAX forecast to 2030Q4
#
# Data     : Corporate_Credit_Unions.xlsx  (monthly total assets)
#            → converted to quarterly end-of-quarter points
#            → merged with macro panel from modeling_panel_v5.rds
#
# Outputs  : results_3c/  (forecasts, metrics, coefficients CSVs)
#            plots_3c/    (PDF plots)
############################################################
...
PLOT_DIR   <- "plots_3c_v5"
RESULT_DIR <- "results_3c_v5"
...
# Corporate CU data starts 2019 Q1; YoY available from 2020 Q1
TRAIN_END  <- zoo::as.yearqtr("2022 Q1")

# TSCV settings
TSCV_MIN_TRAIN        <- 8L    # minimum obs (shorter series: 2019-2025)
TSCV_MIN_TRAIN_SPARSE <- 8L   # not used in 3c (single series)    # floor for sparse categories (7_10B_Plus, 1_Less_10M)
...
SPARSE_CATS <- character(0)  # single series, no sparse categories
...
# 2. LOAD DATA
# ════════════════════════════════════════════════════════════
message("\n[1] Loading Corporate CU data + macro panel...")

# ── 2a. Read Corporate CU total assets from Excel ──────────
CORP_XLSX <- file.path(DATA_DIR, "Corporate_Credit_Unions.xlsx")
if (!file.exists(CORP_XLSX))
  stop("Corporate_Credit_Unions.xlsx not found in DATA_DIR.")

corp_raw <- as.data.table(read_excel(CORP_XLSX))
message(sprintf("    Excel: %d rows × %d cols", nrow(corp_raw), ncol(corp_raw)))

# ── Column mapping (known from Excel inspection) ─────────
# Column A = "date"  (Cycle Date, monthly end-of-month)
# Column BR = "ccu_total_assets" (Corporate CU Total Assets)
date_col  <- "date"
asset_col <- "ccu_total_assets"

# Fallback: if exact names aren't found, search flexibly
if (!date_col %in% names(corp_raw)) {
  date_col <- grep("date|cycle", names(corp_raw), ignore.case=TRUE, value=TRUE)[1L]
  if (is.na(date_col)) date_col <- names(corp_raw)[1L]
}
if (!asset_col %in% names(corp_raw)) {
  asset_col <- grep("total.*asset|ccu.*asset", names(corp_raw), ignore.case=TRUE, value=TRUE)[1L]
  if (is.na(asset_col)) stop("Cannot find ccu_total_assets column in Excel.")
}
...
if (inherits(raw_dates, c("POSIXct","POSIXlt","Date"))) {
  # read_excel correctly parsed as datetime
  corp_raw[, date_raw := as.Date(raw_dates)]
} else if (is.numeric(raw_dates)) {
  # Excel serial date number (origin = 1899-12-30)
  corp_raw[, date_raw := as.Date(raw_dates, origin = "1899-12-30")]
} else {
  # Character — could be serial numbers as text OR formatted date strings
  raw_chr <- trimws(as.character(raw_dates))
  # Check if they look like serial numbers (all digits, 4-6 chars)
  if (all(grepl("^[0-9]+\\.?[0-9]*$", head(raw_chr[!is.na(raw_chr)], 10)))) {
    # Excel serial dates stored as character
    corp_raw[, date_raw := as.Date(as.numeric(raw_chr), origin = "1899-12-30")]
    message("    [DATE] Parsed as Excel serial numbers (character → numeric → Date)")
  } else {
    # Try common date string formats
    corp_raw[, date_raw := as.Date(raw_chr, format = "%m/%d/%Y")]
    if (all(is.na(corp_raw$date_raw)))
      corp_raw[, date_raw := as.Date(raw_chr, format = "%Y-%m-%d")]
    if (all(is.na(corp_raw$date_raw)))
      corp_raw[, date_raw := as.Date(raw_chr, format = "%m-%d-%Y")]
    if (all(is.na(corp_raw$date_raw)))
      corp_raw[, date_raw := as.Date(raw_chr, format = "%d/%m/%Y")]
  }
}
# Diagnostic
n_parsed <- sum(!is.na(corp_raw$date_raw))
message(sprintf("    Dates parsed: %d / %d", n_parsed, nrow(corp_raw)))
if (n_parsed == 0L) {
  message(sprintf("    Raw date sample: %s", paste(head(raw_dates, 5), collapse=", ")))
  message(sprintf("    Raw date class: %s", paste(class(raw_dates), collapse=", ")))
  stop("Date parsing failed. Check Cycle Date column format.")
}
...
message(sprintf("    Date range: %s → %s  (%d monthly obs)",
                min(corp_raw$date_raw), max(corp_raw$date_raw), nrow(corp_raw)))

# ── 2b. Convert monthly to quarterly (quarter-end months) ──
...
corp_qtr <- corp_qtr[, .(date, corp_cu_assets)]
setDT(corp_qtr)
# Deduplicate — keep last observation per quarter
corp_qtr <- corp_qtr[, .(corp_cu_assets = tail(corp_cu_assets, 1L)), by=date]
setDT(corp_qtr)
setorderv(corp_qtr, "date")
message(sprintf("    Quarterly points: %d", nrow(corp_qtr)))

# ── 2c. Create YoY dependent variable ─────────────────────
...
# ── 2d. Add synthetic category (single series — no real categories) ──
# We use a single category "Corporate" to keep the same code structure
corp_qtr[, categories := "Corp"]
corp_qtr[, cat_label  := "Corporate"]

message(sprintf("    YoY obs (non-NA): %d",
                sum(!is.na(corp_qtr$yoy_corp_cu_assets_pct))))

# ── 2e. Load macro panel and merge ────────────────────────
if (!file.exists("modeling_panel_v5.rds"))
  stop("modeling_panel_v5.rds not found. Run 1_Data_Prep_v5.R + 2_Macro_v5.R first.")

macro_panel <- readRDS("modeling_panel_v5.rds")
setDT(macro_panel)

# Strip haven labels
if (requireNamespace("haven", quietly = TRUE)) {
  macro_panel <- as.data.table(haven::zap_labels(macro_panel))
}
for (cn in names(macro_panel)) {
  if (!is.null(attr(macro_panel[[cn]], "label")))  attr(macro_panel[[cn]], "label")  <- NULL
  if (!is.null(attr(macro_panel[[cn]], "labels"))) attr(macro_panel[[cn]], "labels") <- NULL
}

# Extract unique macro columns (one row per quarter — take any category)
macro_cols <- grep("^macro_", names(macro_panel), value=TRUE)
macro_dt   <- unique(macro_panel[, c("date", macro_cols), with=FALSE], by="date")
setorderv(macro_dt, "date")
message(sprintf("    Macro panel: %d quarters × %d macro features",
                nrow(macro_dt), length(macro_cols)))

# Merge corporate CU data with macro features
qtrly <- merge(corp_qtr, macro_dt, by="date", all.x=TRUE)
setorderv(qtrly, "date")

# Ensure data.table class after merge
setDT(qtrly)

# Add seasonal dummies (extract quarter number from yearqtr)
# yearqtr: Q1=.0, Q2=.25, Q3=.5, Q4=.75 → cycle() gives 1-4
qtrly[, qtr := as.integer(cycle(zoo::as.yearqtr(date)))]
for (qq in 1:3) qtrly[, paste0("q", qq) := as.integer(qtr == qq)]

message(sprintf("    Merged panel: %d rows × %d cols",
                nrow(qtrly), ncol(qtrly)))

all_quarters <- sort(unique(qtrly$date))
cats         <- "Corporate"   # Single category
...
DEP_VARS <- list(
  yoy_corp_cu_assets_pct = list(label="Corp CU Assets YoY % Change", short="corp_cu_assets")
)

# ── Exit variables: NOT APPLICABLE for Corporate CUs ─────
# Corporate CUs do not have the same merger/liquidation/acquisition
# dynamics as natural-person CUs.  Exit variables are excluded
# from the 3c model entirely.
EXIT_VARS_RAW <- character(0)
EXIT_VARS     <- character(0)

# Exit variable sign priors — empty (no exit vars)
EXIT_SIGN_PRIOR <- list()  # no exit vars for corporate CUs
...
# Hard exclusions — target itself and any leakage from other models
# For 3c (corporate CU): exclude target, raw levels, and all FCU/FISCU vars
HARD_EXCL <- c("yoy_corp_cu_assets_pct",
               "corp_cu_assets","corp_cu_assets_lag4",
               "yoy_fcu_assets_pct","yoy_fiscu_assets_pct","yoy_assets_pct",
...
# ── Exit variable sign priors (GLOBAL) ────────────────────────
# Corporate CUs have no exit variables, so this is empty.
# The check_exit_signs() function will simply be a no-op.
EXIT_SIGN_PRIOR <- list()
...
fit_window_3c <- function(train_dt, test_row, dep_var, feats,
  ...
  # ─────────────────────────────────────────────────────────────
  # EXIT_SIGN_PRIOR is defined globally above fit_window_3c
  ...
  # ── Step 4c: Significance filter — drop insignificant xreg ──
  #
  # TSCV backward elimination optimises OOS RMSE, not p-values,
  # so variables with p >= SIG_LEVEL can survive.  These add noise
  # to coefficients and produce unreliable forecasts.  We iteratively
  # drop the LEAST significant variable (highest p-value) and refit
  # until all remaining xreg have p < SIG_LEVEL.
  # ─────────────────────────────────────────────────────────────
  if (length(sig_vars) > 0L && !is.null(arimax_final)) {
    MAX_SIG_ITER <- 10L
    for (sig_iter in seq_len(MAX_SIG_ITER)) {
      pvals <- xreg_pvals(arimax_final, n_obs)
      if (length(pvals) == 0L) break

      # Only check xreg variables (not ARIMA terms)
      xreg_pv <- pvals[names(pvals) %in% sig_vars]
      if (length(xreg_pv) == 0L) break

      worst_p <- max(xreg_pv, na.rm = TRUE)
      if (!is.finite(worst_p) || worst_p < SIG_LEVEL) break  # all significant

      worst_var <- names(which.max(xreg_pv))
      message(sprintf("        [SIG FILTER iter %d] dropping '%s' (p=%.4f >= %.2f)",
                      sig_iter, worst_var, worst_p, SIG_LEVEL))
      sig_vars <- setdiff(sig_vars, worst_var)

      if (length(sig_vars) == 0L) {
        arimax_final <- arima_base
        message("        [SIG FILTER] all xreg dropped — using pure ARIMA")
        break
      }

      res_sig <- refit_fn(sig_vars)
      if (!is.null(res_sig$fit)) {
        arimax_final <- res_sig$fit
        sig_vars     <- res_sig$vars
      } else {
        arimax_final <- arima_base
        sig_vars     <- character(0)
        message("        [SIG FILTER] refit failed — falling back to pure ARIMA")
        break
      }
    }

    # Rebuild coef_dt after significance filtering
    coef_dt_final <- build_coef_dt(arimax_final, n_obs, sig_vars)
    adj_r2_final  <- compute_adj_r2(arimax_final, y_train_w)
  }
...
message(sprintf("    Training window start : 2019 Q1"))
...
for (dv in names(DEP_VARS)) {
  ...
  for (cat in cats) {
    ...
    for (tq in test_quarters) {
      train_idx <- cat_dt$date >= zoo::as.yearqtr("2019 Q1") &
                   cat_dt$date <  tq
      ...
      res <- fit_window_3c(train_dt, test_row, dv, feats_dv,
...
fwrite(forecasts_all, file.path(RESULT_DIR, "forecasts_3c_v5.csv"))
fwrite(metrics_all,   file.path(RESULT_DIR, "metrics_3c_v5.csv"))
fwrite(coefs_all,     file.path(RESULT_DIR, "coefficients_3c_v5.csv"))
...
if (nrow(arima_oos_all)>0)
  fwrite(arima_oos_all, file.path(RESULT_DIR, "arima_oos_benchmark_3c_v5.csv"))

message("    Saved CSVs to results_3c/")
...
full_levels <- qtrly[, .(cat_label, date, corp_cu_assets)]
...
make_level_fc <- function(dv, count_col) {
  ...
  lv_sub[, anchor       := data.table::shift(asset_val, n=4L, type="lag"), by=cat_label]
...
level_corp <- make_level_fc("yoy_corp_cu_assets_pct", "corp_cu_assets")
...
fwrite(level_fc,      file.path(RESULT_DIR, "level_forecasts_3c_v5.csv"))
fwrite(level_metrics, file.path(RESULT_DIR, "level_metrics_3c_v5.csv"))
...
# Ensure future_fc has expected columns even if empty
if (nrow(future_fc) == 0L) {
  future_fc <- data.table(dep_var=character(), cat_label=character(),
    series=character(), date=numeric(), pred_yoy=numeric(),
    lo95_yoy=numeric(), hi95_yoy=numeric(), pred_level=numeric(),
    lo95_level=numeric(), hi95_level=numeric(), actual_level=numeric(),
    method_used=character())
}
if (nrow(future_fc) > 0 && "date" %in% names(future_fc)) future_fc[, date := zoo::as.yearqtr(as.numeric(get("date")))]
fwrite(future_fc, file.path(RESULT_DIR, "future_forecast_3c_v5.csv"))
...
#   • Full history from 2019 Q1 — 7 categories, solid lines
...
# ════════════════════════════════════════════════════════════
message("\n[8] Policy-decision chart (P9 Corp CU)...")
...
make_policy_chart_3c <- function(count_col, title_text, stem) {

  # Historical levels from 2019 Q1
  hist_dt <- qtrly[!is.na(get(count_col)) &
                     date >= zoo::as.yearqtr("2019 Q1"),
  ...
  # Coerce date columns to plain numeric to avoid yearqtr class mismatch
  if ("date" %in% names(fc_oos)) fc_oos <- fc_oos[, date := as.numeric(date)]
  if ("date" %in% names(fc_fut)) fc_fut <- fc_fut[, date := as.numeric(date)]
  fc_all <- tryCatch(
    rbindlist(list(fc_oos, fc_fut), fill = TRUE, use.names = TRUE),
    error = function(e) {
      # Fallback: strip all attributes and retry
      for (cn in intersect(names(fc_oos), names(fc_fut))) {
        if (is.numeric(fc_oos[[cn]])) set(fc_fut, j=cn, value=as.numeric(fc_fut[[cn]]))
      }
      rbindlist(list(fc_oos, fc_fut), fill = TRUE, use.names = TRUE)
    })
...
# P9: Corporate CU policy chart (single series)
if ("corp_cu_assets" %in% names(qtrly)) {
  message("    P9: Corp CU Assets policy chart...")
  make_policy_chart_3c(
    count_col  = "corp_cu_assets",
    title_text = "Corporate CU Assets — Historical Trend & ARIMAX Forecast",
    stem       = "P9_policy_corp_cu_assets.pdf"
  )
} else {
  message("    P9: skipped - corp_cu_assets not in data")
}

message("    Policy chart complete (P9)")

# ════════════════════════════════════════════════════════════
# P11: FORECAST PANELS
#
# Single PDF for corporate CU assets. Panel shows:
#   • Historical actual from 2019 Q1           (solid blue)
...
combine_panels <- function(plot_list, ncol = 1,
...
# ── Config ────────────────────────────────────────────────────
PLOT_START_P11 <- zoo::as.yearqtr("2019 Q1")
...
# ── Series metadata ───────────────────────────────────────────
series_meta_3c <- list(
  corp_cu_assets = list(label = "Corp CU Assets", hist_col = "corp_cu_assets",
                        short = "corp_cu_assets", dep_var = "yoy_corp_cu_assets_pct")
)
...
for (sr in names(series_meta_3c)) {
  sm   <- series_meta_3c[[sr]]
  hcol <- sm$hist_col
  ...
  for (cat in cats) {
    ...
    y_ts <- ts(cat_hist[[hcol]], frequency = 4L,
               start = c(as.integer(format(as.Date(min(cat_hist$date)), "%Y")),
                         as.integer(cycle(min(cat_hist$date)))))
    ...
    # ── Guard 2: detect flat or trend-contradicting forecasts ──
    # A perfectly flat forecast (all values identical, or range < 2%
    # of last observed) is NEVER realistic for asset levels.
    # Also catch cases where forecast direction contradicts recent trend.
    ...
    # Trigger if: (a) forecast is flat/constant, OR
    #             (b) recent trend is up but forecast is down/flat, OR
    #             (c) recent trend is down but forecast is up/flat
    needs_refit <- is_flat ||
                   (trend_dir > 0 && fc_dir <= 0) ||
                   (trend_dir < 0 && fc_dir >= 0)

    if (needs_refit) {
      message(sprintf(
        "      [TREND GUARD] %s | %s: recent_slope=%+.0f/qtr  fc_slope=%+.0f/qtr  flat=%s — attempting refit",
        sr, cat, recent_slope, fc_slope, is_flat))

      # ── Attempt 1: auto.arima with forced d=1 ─────────────
      fit_new <- NULL
      fc_new  <- NULL
      ...
      if (!is.null(fit_d1)) {
        fc_d1 <- tryCatch(
          forecast::forecast(fit_d1, h = HORIZON_QTR, level = 95),
          error = function(e) NULL)
        if (!is.null(fc_d1)) {
          d1_vals  <- as.numeric(fc_d1$mean)
          d1_range <- max(d1_vals) - min(d1_vals)
          d1_flat  <- (d1_range / max(abs(last_val), 1)) < 0.02
          if (!d1_flat) {
            fit_new <- fit_d1; fc_new <- fc_d1
            message("      [TREND GUARD] auto.arima(d=1) produced non-flat forecast")
          }
        }
      }

      # ── Attempt 2: explicit ARIMA(1,1,0) + drift ──────────
      if (is.null(fit_new)) {
        message("      [TREND GUARD] trying ARIMA(1,1,0)+drift")
        fit_rw <- tryCatch(
          forecast::Arima(y_ts, order = c(1L, 1L, 0L),
                         include.drift = TRUE),
          error = function(e) NULL)
        if (!is.null(fit_rw)) {
          fc_rw <- tryCatch(
            ...
            error = function(e) NULL)
          if (!is.null(fc_rw)) {
            rw_vals  <- as.numeric(fc_rw$mean)
            rw_range <- max(rw_vals) - min(rw_vals)
            rw_flat  <- (rw_range / max(abs(last_val), 1)) < 0.02
            if (!rw_flat) {
              fit_new <- fit_rw; fc_new <- fc_rw
              message("      [TREND GUARD] ARIMA(1,1,0)+drift OK")
            }
          }
        }
      }

      # ── Attempt 3: ARIMA(0,1,1) + drift ───────────────────
      if (is.null(fit_new)) {
        message("      [TREND GUARD] trying ARIMA(0,1,1)+drift")
        fit_ma <- tryCatch(
        ...
        if (!is.null(fit_ma)) {
          fc_ma <- tryCatch(
            forecast::forecast(fit_ma, h = HORIZON_QTR, level = 95),
            error = function(e) NULL)
          if (!is.null(fc_ma)) {
            fit_new <- fit_ma; fc_new <- fc_ma
            message("      [TREND GUARD] ARIMA(0,1,1)+drift OK")
          }
        }
      }

      # ── Attempt 4: ARIMA(0,2,1) — linear trend via d=2 ────
      if (is.null(fit_new)) {
        message("      [TREND GUARD] trying ARIMA(0,2,1) for linear trend")
        fit_d2 <- tryCatch(
          forecast::Arima(y_ts, order = c(0L, 2L, 1L)),
          error = function(e) NULL)
        if (!is.null(fit_d2)) {
          fc_d2 <- tryCatch(
            forecast::forecast(fit_d2, h = HORIZON_QTR, level = 95),
            error = function(e) NULL)
          if (!is.null(fc_d2)) {
            fit_new <- fit_d2; fc_new <- fc_d2
            message("      [TREND GUARD] ARIMA(0,2,1) OK")
          }
        }
      }

      # Accept the refit if we found one
      if (!is.null(fit_new) && !is.null(fc_new)) {
        fit_a <- fit_new
        fc_a  <- fc_new
        new_vals <- as.numeric(fc_new$mean)
        message(sprintf("      [TREND GUARD] ACCEPTED: forecast range $%.0fM–$%.0fM",
                        min(new_vals)/1e6, max(new_vals)/1e6))
      } else {
        message(sprintf("      [TREND GUARD] WARNING: all refits failed for %s | %s — keeping original flat forecast",
                        sr, cat))
      }
    }
...
if (nrow(arima_base_all) > 0) {
  ...
  csv_path <- file.path(RESULT_DIR, "pure_arima_forecasts_3c.csv")
  ...
  if (!has_openxlsx) {
    ...
    tryCatch({
      ...
      xlsx_path <- file.path(RESULT_DIR, "pure_arima_forecasts_3c.xlsx")
...
for (sr in names(series_meta_3c)) {
  ...
  combined <- patchwork::wrap_plots(panel_plots, ncol = 1) +
    patchwork::plot_annotation(
      title    = sprintf("%s \u2014 Actual vs ARIMAX (rolling OOS) vs ARIMA (future)",
                         label),
      subtitle = sprintf(
        paste("Historical: 2019Q1 \u2192 %s  |  Amber region: forecast 2025Q4\u20132030Q4",
  ...
  tryCatch({
    pdf_dev <- tryCatch(
      { grDevices::cairo_pdf(pdf_path, width = 12, height = 8); "cairo" },
      error = function(e) { grDevices::pdf(pdf_path, width = 12, height = 8); "pdf" })
    print(combined)
    grDevices::dev.off()
...
for (sr in names(series_meta_3c)) {
  ...
  tryCatch({
    pdf_dev <- tryCatch(
      { grDevices::cairo_pdf(hm_path, width = 12, height = 5); "cairo" },
      error = function(e) { grDevices::pdf(hm_path, width = 12, height = 5); "pdf" })
    print(p_hm)
    grDevices::dev.off()
...
# ════════════════════════════════════════════════════════════
message("\n[10b] P11: Historical + Pure ARIMA panels...")
...
for (sr in names(series_meta_3c)) {
  ...
  combined_ab <- patchwork::wrap_plots(panel_plots_ab, ncol = 1) +
    patchwork::plot_annotation(
      title    = sprintf("%s \u2014 Historical + Pure ARIMA Forecast", label),
      subtitle = sprintf(
        "Historical: 2019Q1 \u2192 %s  |  Amber: forecast region  |  Horizon markers: +1yr / +3yr / +5yr",
        as.character(LAST_OBS)),
      theme = theme(
        plot.title    = element_text(face = "bold", size = 14),
        plot.subtitle = element_text(size = 9, colour = "grey30"))
  ...
  tryCatch({
    pdf_dev <- tryCatch(
      { grDevices::cairo_pdf(pdf_path_ab, width = 12, height = 8); "cairo" },
      error = function(e) { grDevices::pdf(pdf_path_ab, width = 12, height = 8); "pdf" })
    print(combined_ab)
    grDevices::dev.off()
...
if (nrow(arima_oos_all) > 0 && nrow(forecasts_all) > 0) {
  ...
  # ── Open PDF device ─────────────────────────────────────────
  pdf_ok <- tryCatch({
    grDevices::pdf(oos_pdf_path, width=12, height=8, onefile=TRUE)
    TRUE
...
pdf_ok14 <- tryCatch({
  grDevices::pdf(p14_path, width=12, height=8, onefile=TRUE)
  TRUE
...
pdf_ok15 <- tryCatch({
  grDevices::pdf(p15_path, width=12, height=8, onefile=TRUE)
  TRUE
...
# ════════════════════════════════════════════════════════════
# ════════════════════════════════════════════════════════════
# P16: PURE ARIMA MEAN FORECAST HEATMAPS  (point only)
# P16_CI: PURE ARIMA 95% CI HEATMAPS       (lo95 + hi95)
# ════════════════════════════════════════════════════════════
...
if (nrow(arima_base_all) == 0) {
  ...
  for (sr in names(series_meta_3c)) {
    ...
    tryCatch({
      pdf_dev <- tryCatch(
        { grDevices::cairo_pdf(hm_path_mean, width=12, height=4); "cairo" },
        error=function(e) { grDevices::pdf(hm_path_mean, width=12, height=4); "pdf" })
      print(p_mean)
      grDevices::dev.off()
    ...
    tryCatch({
      pdf_dev <- tryCatch(
        { grDevices::cairo_pdf(hm_path_ci, width=12, height=5); "cairo" },
        error=function(e) { grDevices::pdf(hm_path_ci, width=12, height=5); "pdf" })
      print(p16_ci)
      grDevices::dev.off()
...
message("\n[12] Done. All outputs in results_3c/ and plots_3c/")
...
message(paste("    Plots:",
              ...
              "P11 (hist+ARIMA only) |",
              "P16 (ARIMA mean heatmaps) |",
              "P16_CI (ARIMA CI heatmaps)"))
