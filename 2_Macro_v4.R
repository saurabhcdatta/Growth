...
#
# FRB variable set (from spreadsheet, all 119 series):
#   Rates / monetary policy
#     RFF RFFEFF RFFP RS3M RS3Y RS7Y RS10Y RS10Y6M RS30Y
#     RT1Y RT3Y RT7Y RT10Y RT20Y RT30Y RMTG RPRM
#     MPE2Y MPE10Y MPE310Y RTB RT10YFED RT5YFED SOFRRATE RT1M
#     RF1Y1Y..RF1Y9Y  RF5Y1Y..RF5Y5Y
#   Spreads
#     SRCB SRCBBB10 RCBBB SRMTG SRPRM SRIOR SRDIS
#     SONOFF SONOFFS SSOFRRATE
#   Labour
#     LURC NAIRU LFPRC LFPRCDEM LUAD AWUBS LIC
#   Output / activity
#     GDPS IP IPMFG KBKCS
#     ECS ECDS ECDMVS ECNDS ECNDFS ECNDGS ECONS ECONCDMVS ECSS EFIS
#   Prices / inflation
#     PCPI PCPIXFE PPI PBRENT PBXE PHPI PI_STAR MU
#   Housing
#     HMI6M HP1 CASC20XA USHPI USPHPI RHS RHM RPS
#     UFOREINV UFORES PHPINEW PCREPI
#   Financial markets
#     MCAP500S PPSDJT PPSDJIA PPSNASDAQ MSCISC
#   Credit quality
#     CCDQ CCCO OCLDQ OCLCO CILDQ CILCO ULEV HDNNB
#   Consumer credit & debt
#     CRC CNRC TCC MORTGAGES
#   Household finance
#     YPDS YLABORS YPIPCB YPINTS YPDIVS YNIMPS UYPSAV UYUPRR
#     CCONF FRM15
#
# Derived series (built only from FRB columns):
#   yield_curve      = RS10Y - RS3M
#   spread_2s10s     = RS10Y - RS3Y
#   yield_curve_inv / yield_curve_inv_run
#   cpi_yoy          = PCPI YoY %
#   core_cpi_yoy     = PCPIXFE YoY %
#   real_rate        = RFF - cpi_yoy
#   hy_spread        = SRCB  (alias; Part-3 FEAT_PAT expects this name)
#   credit_tightness = avg z-score(SRCB, SRCBBB10, CCDQ, CILDQ)
#   fedfunds_cycle   = RFF - trailing-8q mean
#   fedfunds_chg / fomc_regime / hike_run
#   oil_qoq_pct / oil_shock  (from PBRENT)
#   Interaction terms: rate_x_slope, rate_x_hy, rate_x_credit,
#                      unemp_x_real_rate, slope_x_credit
#
# Feature engineering applied to all base FRB columns:
#   yoy_{v}            YoY % change  (lag-4)
#   qoq_{v}            QoQ % change  (lag-1)
#   {v}_rmean4/8       rolling mean  4q / 8q
#   {v}_rsd4           rolling SD    4q
#   {v}_cyc            cyclical deviation  (v - rmean8)
#   yoy_{v}_accel      YoY acceleration   (delta yoy, lag-4)
#   {v}_lag1/2/4       lagged levels
#
# Output column names match macro_v3_arima.R — Part 3 unchanged.
#
# Outputs:
#   macro_raw_v4.rds        raw FRB table  (yearqtr date)
#   macro_features_v4.rds   engineered     (hist + projection)
#   macro_forecast_v4.csv   projection rows, key series
#   qtrly_enriched_v3.rds   overwritten with v4 macro
#   qtrly_full_v3.rds       overwritten with v4 macro
#   plots_v4/               M01-M12  PDF diagnostics
#
# Runtime: < 2 min
############################################################
...
PLOT_FROM <- 1990L   # earliest year shown in charts

# ============================================================
# 2. VARIABLE MAP  (FRB column name -> internal short name)
#    ONLY FRB columns that exist in the spreadsheet.
#    No FRED series. No external proxies.
# ============================================================
VAR_MAP <- c(
  # Rates & monetary policy
  "RFF"        = "fedfunds",
...
# Parse 1975.1 / 1975Q1 -> zoo yearqtr
...
# Device-safe PDF save: tries pdf() device directly (no cairo dependency)
...
message(sprintf("Historical  : 1975 Q1 -> %s", as.character(HIST_END)))
...
for (old_nm in intersect(names(frb_raw), names(VAR_MAP))) {
  ...
  if (new_nm %in% names(frb_raw) && new_nm != old_nm) {
    message(sprintf("    SKIP '%s'->'%s': target name already exists", old_nm, new_nm))
    next
  }
...
# ---- is_forecast flag  (logical) ---------------------------
frb_raw[, is_forecast := (date > HIST_END)]

# ---- Stamp yearqtr class + assert --------------------------
frb_raw[, date := zoo::as.yearqtr(date)]
stopifnot("date" %in% names(frb_raw),
          inherits(frb_raw$date, "yearqtr"))
...
message(sprintf("    Historical: %d rows  |  Projection: %d rows",
                sum(!frb_raw$is_forecast), sum(frb_raw$is_forecast)))
message(sprintf("    Variables : %d  (after renaming)", ncol(frb_raw) - 2L))

saveRDS(frb_raw, file.path(DATA_DIR, "macro_raw_v4.rds"))
message("    Saved     : macro_raw_v4.rds")
toc()

# ============================================================
# SECTION 2: DERIVED / COMPOSITE SERIES
#   Built ONLY from FRB columns. No FRED proxies.
# ============================================================
tic("S2. Derived series")
message("\n[S2] Derived / composite series...")

m <- copy(frb_raw)
m[, date := zoo::as.yearqtr(date)]   # re-stamp after copy()

# -- Yield curve: RS10Y - RS3M ------------------------------
if (all(c("gs10", "gs3m") %in% names(m))) {
  m[, yield_curve := gs10 - gs3m]
  message("    yield_curve      = gs10 - gs3m  (RS10Y - RS3M)")
}

# -- 2s10s spread: RS10Y - RS3Y -----------------------------
if (all(c("gs10", "gs3") %in% names(m))) {
  m[, spread_2s10s := gs10 - gs3]
  message("    spread_2s10s     = gs10 - gs3   (RS10Y - RS3Y)")
}
...
if ("cpi" %in% names(m)) {
  ...
  message("    cpi_yoy          (PCPI YoY %)")
}

# -- Core CPI YoY (PCPIXFE -> core_cpi) --------------------
if ("core_cpi" %in% names(m)) {
  m[, core_cpi_yoy := pct_chg_n(core_cpi, 4L)]
  message("    core_cpi_yoy     (PCPIXFE YoY %)")
}

# -- Real fed funds rate: RFF - cpi_yoy ---------------------
if (all(c("fedfunds", "cpi_yoy") %in% names(m))) {
  m[, real_rate := fedfunds - cpi_yoy]
  message("    real_rate        = fedfunds - cpi_yoy")
}

# -- hy_spread alias: SRCB = baa_spread ---------------------
# Part-3 FEAT_PAT searches for the name 'hy_spread'.
# FRB has SRCB (mapped to baa_spread). No true HY OAS in FRB.
if ("baa_spread" %in% names(m)) {
  m[, hy_spread := baa_spread]
  message("    hy_spread        <- baa_spread  (SRCB alias for Part-3)")
}

# -- Credit tightness: avg z-score of FRB credit series ----
ct_inputs <- intersect(
...
#   Applied to every base FRB numeric column.
#   Composites created in S2 are excluded from generic FE.
# ============================================================
tic("S3. Feature engineering")
message("\n[S3] Feature engineering (YoY / QoQ / rolling / lags)...")

# Columns already computed in S2 — skip generic FE on these
DERIVED_COLS <- c(
  "date", "is_forecast",
...
# Key series for rolling stats and lags
# Strictly limited to FRB-sourced columns (internal names after VAR_MAP)
# plus derived composites that are meaningful for rolling windows.
# NOTE: hy_spread is an alias of baa_spread — only include baa_spread here.
KEY_MACRO <- intersect(c(
  ...
  # Yield curve & spreads (FRB)
  "yield_curve", "spread_2s10s",
  "baa_spread", "bbb10_spread", "mtg_spread", "prime_spread",
  # Derived composites (meaningful for rolling)
...
# Re-stamp date class (always safe)
m[, date := zoo::as.yearqtr(date)]
stopifnot("date" %in% names(m), inherits(m$date, "yearqtr"))
message(sprintf("    FE done: %d rows x %d cols  |  date: %s -> %s",
                nrow(m), ncol(m),
...
# NA rate on historical rows only
...
# Critical Part-3 column check
p3_req   <- c("fedfunds", "gs10", "unrate", "cpi_yoy",
...
saveRDS(m, file.path(DATA_DIR, "macro_features_v4.rds"))
message(sprintf("    macro_features_v4.rds  (%d x %d)  %s -> %s",
                nrow(m), ncol(m),
                as.character(min(m$date)), as.character(max(m$date))))

# CSV: projection rows, key series + human-readable date columns
key_exp <- intersect(
  c("date", KEY_MACRO,
...
setcolorder(fc_exp,
            c("date", "date_label", "year", "qtr",
              setdiff(names(fc_exp),
                      c("date", "date_label", "year", "qtr"))))
fwrite(fc_exp, file.path(DATA_DIR, "macro_forecast_v4.csv"))
message(sprintf("    macro_forecast_v4.csv  (%d rows x %d cols)",
                nrow(fc_exp), ncol(fc_exp)))
toc()

# ============================================================
# SECTION 6: MERGE ONTO CU QUARTERLY PANELS
# ============================================================
tic("S6. Merge")
message("\n[S6] Merging onto CU quarterly panels...")

merge_cols <- setdiff(names(m), c("date", "is_forecast"))

merge_macro_v4 <- function(panel_path, out_path, use_forecast) {
  if (!file.exists(panel_path)) {
    message(sprintf("    SKIP (not found): %s", panel_path))
    return(invisible(NULL))
  }
  panel <- readRDS(panel_path)
  setDT(panel)
  # Ensure panel date is yearqtr for matching
  if ("date" %in% names(panel) && !inherits(panel$date, "yearqtr"))
    panel[, date := zoo::as.yearqtr(date)]

  message(sprintf("    %s: %d x %d (before merge)",
                  basename(panel_path), nrow(panel), ncol(panel)))

  macro_src <- if (use_forecast) m else m[is_forecast == FALSE]

  # Remove stale macro columns
  ...
  if (n_match == 0) {
    message("    ERROR: 0 date matches. Verify panel$date is yearqtr.")
    return(invisible(NULL))
  }
...
merge_macro_v4(file.path(DATA_DIR, "qtrly_enriched_v3.rds"),
              file.path(DATA_DIR, "qtrly_enriched_v3.rds"),
              use_forecast = FALSE)   # historical only
merge_macro_v4(file.path(DATA_DIR, "qtrly_full_v3.rds"),
              file.path(DATA_DIR, "qtrly_full_v3.rds"),
              use_forecast = TRUE)    # hist + projection
toc()
...
#
# Design decisions:
#   - date_d  (Date)  is pre-computed once on m_plot
#   - yr_int  (int)   filters by PLOT_FROM without lubridate::year()
#   - is_forecast is converted to character for colour scale
#     to guarantee ggplot2 legend labels match exactly
#   - save_plot() uses pdf() device directly (no cairo dependency)
# ============================================================
tic("S7. Plots")
message("\n[S7] Generating diagnostic plots...")

# Pre-compute Date and year columns on a plot copy
m_plot <- copy(m)
m_plot[, date_d  := as.Date(date)]
m_plot[, yr_int  := as.integer(format(date, "%Y"))]
# Convert logical to character for reliable colour scale mapping
...
plot_ts <- function(vname, y_label, log_sc = FALSE, stem = NULL) {
  if (!vname %in% names(m_plot)) {
    message(sprintf("    SKIP (not in data): %s", vname))
    return(invisible(NULL))
  }
  dt <- m_plot[yr_int >= PLOT_FROM & !is.na(get(vname)),
               .(date_d, y = get(vname), period)]
  if (nrow(dt) == 0) {
    message(sprintf("    SKIP (all NA): %s", vname))
    return(invisible(NULL))
  }
  p <- ggplot(dt, aes(x = date_d, y = y)) +
    annotate("rect",
             xmin = FC_START_D, xmax = FC_END_D + 100,
             ymin = -Inf, ymax = Inf,
    ...
    labs(title    = paste0(y_label, "  \u2014  FRB Baseline 2026"),
         subtitle = paste0("Blue = historical  |  Red dashed = FRB projection",
                           "  |  Yellow = forecast period"),
         x = NULL, y = y_label) +
    theme_v4()
...
plot_facet <- function(vars, title_txt, stem, ncol = 3, w = 14, h = 7) {
  # Keep only vars that actually exist in the data
  vars <- intersect(vars, names(m_plot))
  if (length(vars) < 2) {
    message(sprintf("    SKIP panel (only %d vars available): %s",
                    length(vars), stem))
    return(invisible(NULL))
  }
  keep_cols <- c("date_d", "yr_int", "period", vars)
  dt <- melt(
  ...
  # Drop all-NA series from this panel
  non_na <- dt[, .(has_data = any(!is.na(value))), by = series]
  dt <- dt[series %in% non_na[has_data == TRUE]$series]
  if (nrow(dt) == 0 || dt[, uniqueN(series)] < 2) {
    message(sprintf("    SKIP panel (no non-NA data): %s", stem))
    return(invisible(NULL))
  }
  ...
  p <- ggplot(dt[!is.na(value)],
    ...
    labs(title    = paste0(title_txt, "  \u2014  FRB Baseline 2026"),
         subtitle = "Yellow = FRB projection period (2025 Q4 \u2013 2030 Q4)",
         x = NULL, y = NULL) +
    theme_v4()
...
plot_ts("mortgage30",  "30-yr Mortgage Rate (RMTG, %)",stem = "M06_mortgage30")

# M07: Monetary policy & yield curve panel ------------------
...
# M09: Credit & financial conditions panel ------------------
...
# M11: Derived / composite indicators ----------------------
...
# M12: Variable coverage heatmap by decade -----------------
message("    M12: Coverage heatmap...")
m_cov <- copy(m)
m_cov[, decade := paste0(
  floor(as.integer(format(date, "%Y")) / 10) * 10, "s")]
...
if (length(cov_vars) >= 5) {
  ...
  p_cov <- ggplot(cov_m, aes(x = decade, y = variable, fill = pct)) +
    ...
    labs(title    = "Variable Coverage by Decade  \u2014  FRB Baseline 2026",
         subtitle = "Green = fully available  |  Red = sparse or absent in FRB file",
         x = NULL, y = NULL) +
    theme_v4() +
...
message(sprintf("Variables  : %d raw FRB -> %d after feature engineering",
...
message(sprintf("  macro_features_v4.rds     %d cols (hist + projection)",
                ncol(m)))
message("  macro_forecast_v4.csv     projection rows with date_label/year/qtr")
