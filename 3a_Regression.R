...
for (dv in names(DEP_VARS)) {
  ...
  for (cat in cats) {
    ...
    if (!is.null(last_res) && last_res$ok) {
      ...
      # ── Always rebuild coef_dt from the actual final fit object ──
      # Guarantees pure ARIMA models show their AR/MA/seasonal/drift coefs
      fit_for_print <- last_res$arimax_fit
      n_for_print   <- last_res$n_train %||% 20L
      cd <- if (!is.null(fit_for_print) &&
                !is.null(fit_for_print$coef) &&
                length(fit_for_print$coef) > 0) {
              build_coef_dt(fit_for_print, n_for_print,
                            last_res$sig_vars %||% character(0))
            } else {
              last_res$coef_dt   # fall back to stored version
            }
  ...
  if (pdf_ok14) {
    ...
    for (ri in seq_len(nrow(pure_models))) {
      ...
      oos_met  <- if (nrow(valid_fc) >= 2L) {
                    ...
                    list(rmse=NA, mae=NA, r2_oos=NA, n=nrow(valid_fc))
                  }
      ...
      oos_dt <- if (nrow(valid_fc) > 0) {
                  ...
                  data.table(date=numeric(0), y=numeric(0), type=character(0))
                }
