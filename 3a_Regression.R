...
for (raw_v in EXIT_VARS_RAW) {
  ...
  if (!lag1_name %in% names(qtrly) && raw_v %in% names(qtrly)) {
    # Compute lag1 per category using base R approach — avoids shift() namespace clash
    qtrly[[lag1_name]] <- unlist(
      lapply(split(qtrly[[raw_v]],
                   qtrly[["cat_label"]]),
             function(x) c(NA_real_, x[-length(x)]))
    )
    message(sprintf("    [LAG1 CREATED] %s", lag1_name))
  }
}
...
fit_window_3a <- function(train_dt, test_row, dep_var, feats,
  ...
  # ─────────────────────────────────────────────────────────────
  EXIT_SIGN_PRIOR <- c(
    merger_rate_lag1      = -1L,   # must be negative
    liquid_rate_lag1      = -1L,   # must be negative
    acquisition_rate_lag1 = -1L,   # must be NEGATIVE (acquisitions remove independent CU)
    exit_rate_lag1        = -1L,   # must be negative
    exit_roll4_lag1       =  0L    # no restriction
  )
    ...
    for (ev in intersect(vars, names(EXIT_SIGN_PRIOR))) {
      ...
      if (wrong_sign || insig) {
        reason <- if (wrong_sign && insig) "wrong sign + insignificant" else
                  if (wrong_sign)          "wrong sign" else "insignificant"
