...
CURATED_MACRO <- c(
  ...
  # Inflation — note: in data these appear as yoy_cpi, yoy_core_cpi
  # so we list both the base name AND the yoy-prefixed version
  "cpi", "core_cpi", "cpi_yoy",
  # Housing & consumer credit
...
# Report which curated base vars were found in the data vs missing.
# Helps diagnose naming mismatches between macro_v4 and part3a.
...
build_coef_dt <- function(fit, n_obs, sig_vars = character(0)) {
  ...
  if (length(cf_all) == 0L) return(data.table())
...
for (dv in names(DEP_VARS)) {
  ...
  for (cat in cats) {
    ...
    if (!is.null(last_res) && last_res$ok) {
      ...
      cat(sprintf("  TSCV RMSE   : %.4f  |  In-sample Adj R²: %.4f\n",
                  last_res$tscv_rmse %||% NA, last_res$adj_r2 %||% NA))
      ...
      if (!is.null(cd) && nrow(cd) > 0) {
        ...
        for (i in seq_len(nrow(cd))) {
          ...
          # Tag variable type — exit vars are now _lag1 names
          ...
          vtype <- if (is_exit_v) "[EXIT]" else
                   if (grepl("^(ar|ma|sar|sma)[0-9]+$|intercept|drift|mean",
                              r$variable)) "[ARIMA]" else "[MACRO]"
          cat(sprintf("  %-28s %12.6f %11.6f %8.3f %11.6f %s  %s\n",
                      r$variable, r$estimate, r$std_err,
                      r$t_stat, r$p_value, stars, vtype))
        }
        cat(sprintf("  %s\n", strrep("-", 74)))
      ...
      for (ev in names(EXIT_SIGN_PRIOR)) {
        if (!ev %in% names(last_res$arimax_fit$coef)) {
          cat(sprintf("    %-20s  not in final model", ev))
          # Explain why
          in_lasso <- if (!is.null(last_res$sig_vars)) ev %in% last_res$sig_vars else FALSE
          if (!in_lasso)
            cat("  (LASSO/TSCV eliminated — did not improve OOS RMSE)\n")
          else
            cat("  (exit-gate dropped: wrong sign or insignificant)\n")
        } else {
          cf_ev  <- last_res$arimax_fit$coef[ev]
          se_ev  <- sqrt(diag(last_res$arimax_fit$var.coef))[
                      match(ev, rownames(last_res$arimax_fit$var.coef))]
          pv_ev  <- 2 * pt(-abs(cf_ev/se_ev),
                            df=max(last_res$n_train - length(last_res$arimax_fit$coef), 1L))
          prior  <- EXIT_SIGN_PRIOR[[ev]]
          sign_ok <- prior == 0L ||
                     (prior == -1L && cf_ev < 0) ||
                     (prior ==  1L && cf_ev > 0)
          cat(sprintf("    %-20s  est=%+.4f  p=%.4f  sign=%s  sig=%s\n",
                      ev, cf_ev, pv_ev,
                      if (sign_ok) "✓ correct" else "✗ WRONG",
                      if (pv_ev < SIG_LEVEL) "✓ sig" else "  n.s."))
        }
      }
      ...
      if (nrow(valid) >= 2L) {
        ...
        perf_note <- if (isTRUE(m_met$r2_oos > 0.5))
          "  → Good: model explains >50% of OOS variance"
        else if (isTRUE(m_met$r2_oos > 0.2))
          "  → Moderate: ARIMA dynamics dominant, macro adds limited lift"
        else if (isTRUE(m_met$r2_oos > 0))
          "  → Weak: minimal OOS predictability beyond naive mean"
        else
          "  → ⚠ Negative R²: worse than naive mean — consider pure ARIMA"
        cat(perf_note, "\n")
      }
...
if (nrow(arima_oos_all)>0)
  fwrite(arima_oos_all, file.path(RESULT_DIR, "arima_oos_benchmark_3a.csv"))
...
series_meta_3a <- list(
  ficu_count  = list(label = "FICU Count",  hist_col = "ficu_count",
                     short = "ficu",  dep_var = "yoy_ficu_pct"),
...
for (sr in names(series_meta_3a)) {
  ...
  hm_pnum <- if (sr == "ficu_count") "P12a" else "P12b"
...
# ════════════════════════════════════════════════════════════
# P14: PURE ARIMA MODELS PDF
#      One page per model that used pure ARIMA (no xreg).
#      Triggered by method_used in:
#        PURE_ARIMA_SPARSE / PURE_ARIMA_NEARCONST /
#        PURE_ARIMA_GLMNET_FALLBACK / ARIMAX_TSCV with 0 final xreg
#
# Each page (4 panels):
#   Top-left  : Actual vs Fitted (in-sample) + 1-step OOS forecasts
#   Top-right : Residuals over time + loess trend
#   Bot-left  : ACF of residuals (diagnostic — should be white noise)
#   Bot-right : Stats box: ARIMA order, Adj R², AIC, OOS RMSE, n
# ════════════════════════════════════════════════════════════
message("\n[14] P14: Pure ARIMA models PDF...")

# Identify which models used pure ARIMA in their FINAL window
pure_arima_methods <- c("PURE_ARIMA_SPARSE", "PURE_ARIMA_NEARCONST",
                        "PURE_ARIMA_GLMNET_FALLBACK")

# A model counts as "pure ARIMA" if:
#   (a) last_res$method_used is one of the above, OR
#   (b) method_used == ARIMAX_TSCV but n_final == 0 (all xreg dropped by gate)
pure_models <- forecasts_all[
  method_used %in% pure_arima_methods |
  (method_used == "ARIMAX_TSCV" & n_final == 0L),
  .(dep_var, cat_label)
] |> unique()

message(sprintf("    Found %d pure-ARIMA model(s)", nrow(pure_models)))

if (nrow(pure_models) > 0) {

  p14_path <- file.path(RESULT_DIR, "P14_pure_arima_models.pdf")
  pdf_ok14 <- tryCatch({
    grDevices::pdf(p14_path, width=14, height=10, onefile=TRUE)
    TRUE
  }, error=function(e) { message("[WARN] P14 PDF open failed: ", e$message); FALSE })

  if (pdf_ok14) {

    for (ri in seq_len(nrow(pure_models))) {
      dv      <- pure_models$dep_var[ri]
      cat_lbl <- pure_models$cat_label[ri]
      ...
      hist_dt <- cat_dt[!is.na(get(dv)), .(date=as.numeric(date), y=get(dv))]

      # ── Fitted values (in-sample) from stored fit ───────────
      fit_obj  <- all_fits[[key]]$fit
      fitted_v <- if (!is.null(fit_obj))
                    tryCatch(as.numeric(fitted(fit_obj)), error=function(e) NULL)
                  else NULL
      resid_v  <- if (!is.null(fit_obj))
                    tryCatch(as.numeric(residuals(fit_obj)), error=function(e) NULL)
                  else NULL
      ...
      oos_met  <- if (nrow(valid_fc) >= 2L)
                  ...
                  else list(rmse=NA, mae=NA, r2_oos=NA, n=nrow(valid_fc))

      # ── Adj R² and AIC from fit ─────────────────────────────
      adj_r2_v <- if (!is.null(fit_obj))
                    compute_adj_r2(fit_obj, as.numeric(na.omit(cat_dt[[dv]])))
                  else NA_real_
      aic_v    <- if (!is.null(fit_obj))
                    tryCatch(AIC(fit_obj), error=function(e) NA_real_)
                  else NA_real_
      arima_ord <- if (!is.null(fit_obj))
                     tryCatch(forecast::arimaorder(fit_obj), error=function(e) NULL)
                   else NULL
      ...
      method_lbl <- if (!is.null(fc_sub$method_used))
                      unique(fc_sub$method_used[!is.na(fc_sub$method_used)])[1L]
                    else "PURE_ARIMA"
      ...
      if (!is.null(fitted_v) && length(fitted_v) == nrow(hist_dt)) {
        fit_dt <- data.table(date=hist_dt$date, y=fitted_v, type="Fitted (in-sample)")
        plot_dt2 <- rbindlist(list(plot_dt, fit_dt), fill=TRUE)
      } else {
        plot_dt2 <- plot_dt
      }

      oos_dt <- if (nrow(valid_fc) > 0)
                  data.table(date=as.numeric(valid_fc$date),
                             y=valid_fc$pred_final, type="OOS Forecast")
                else data.table(date=numeric(0), y=numeric(0), type=character(0))

      combo_dt <- rbindlist(list(plot_dt2, oos_dt), fill=TRUE)

      p1 <- ggplot(combo_dt, aes(x=date, y=y, colour=type, linetype=type)) +
        geom_line(linewidth=0.8) +
        geom_point(data=combo_dt[type=="OOS Forecast"], size=2) +
        geom_hline(yintercept=0, linetype="dotted", colour="grey50") +
        scale_colour_manual(
          values=c("Actual"="black", "Fitted (in-sample)"="#2166ac",
                   "OOS Forecast"="#d73027"), name="") +
        scale_linetype_manual(
          values=c("Actual"="solid", "Fitted (in-sample)"="dashed",
                   "OOS Forecast"="dotted"), name="") +
        scale_x_continuous(
          labels=function(x) as.character(zoo::as.yearqtr(x)),
          breaks=pretty(combo_dt$date, n=6)) +
        labs(title="Actual vs Fitted (in-sample) + OOS Forecast",
             x="Quarter", y="YoY %") +
        theme_cu + theme(legend.position="bottom")

      # ── Panel 2: Residuals over time ─────────────────────────
      if (!is.null(resid_v) && length(resid_v) > 0) {
        res_dt <- data.table(
          date = hist_dt$date[seq_along(resid_v)],
          resid = resid_v)
        p2 <- ggplot(res_dt, aes(x=date, y=resid)) +
          geom_hline(yintercept=0, colour="grey50", linetype="dashed") +
          ...
          geom_smooth(method="loess", se=FALSE, colour="#e6550d",
                      linewidth=0.8, span=0.4) +
          scale_x_continuous(
            labels=function(x) as.character(zoo::as.yearqtr(x)),
            breaks=pretty(res_dt$date, n=6)) +
          labs(title="Residuals Over Time  (loess = orange)",
               subtitle="Should be flat near zero — trend indicates misspecification",
               x="Quarter", y="Residual") +
          theme_cu
      } else {
        p2 <- ggplot() +
          labs(title="Residuals — not available") + theme_cu
      }

      # ── Panel 3: ACF of residuals ────────────────────────────
      if (!is.null(resid_v) && length(resid_v) >= 8L) {
        acf_out  <- tryCatch(acf(resid_v, lag.max=16L, plot=FALSE),
                             error=function(e) NULL)
        if (!is.null(acf_out)) {
          acf_dt   <- data.table(lag=as.numeric(acf_out$lag[-1]),
                                 acf=as.numeric(acf_out$acf[-1]))
          ci_bound <- 1.96 / sqrt(length(resid_v))
          p3 <- ggplot(acf_dt, aes(x=lag, y=acf)) +
            geom_hline(yintercept=0, colour="grey50") +
            geom_hline(yintercept= ci_bound, linetype="dashed",
                       colour="#2166ac", linewidth=0.6) +
            geom_hline(yintercept=-ci_bound, linetype="dashed",
                       colour="#2166ac", linewidth=0.6) +
            geom_segment(aes(x=lag, xend=lag, y=0, yend=acf),
                         colour="#31a354", linewidth=1.0) +
            geom_point(colour="#31a354", size=2.5) +
            labs(title="ACF of Residuals",
                 subtitle="Bars outside blue lines = autocorrelation remains",
                 x="Lag (quarters)", y="ACF") +
            theme_cu
        } else {
          p3 <- ggplot() + labs(title="ACF — failed") + theme_cu
        }
      } else {
        p3 <- ggplot() + labs(title="ACF — insufficient residuals") + theme_cu
      }

      # ── Panel 4: Stats box ───────────────────────────────────
      coef_tbl <- if (!is.null(fit_obj)) {
        cd_p14 <- build_coef_dt(fit_obj,
                    length(na.omit(cat_dt[[dv]])))
        if (nrow(cd_p14) > 0) {
          paste(apply(cd_p14, 1, function(r) {
            ...
            sprintf("  %-22s  %+.5f  p=%.4f %s",
                    r["variable"], est, ifelse(is.na(pv),NA,pv), stars)
          }), collapse="
")
...
",
      ...
      p4 <- ggplot(data.frame(x=0,y=0), aes(x,y)) +
        annotate("text", x=0, y=0, label=stats_text,
                 hjust=0, vjust=1, size=3.1, family="mono") +
        expand_limits(x=c(0,1), y=c(-1,0)) +
      ...
      page14 <- (p1 | p2) / (p3 | p4) +
        patchwork::plot_annotation(
          title    = sprintf("Pure ARIMA Model  |  %s  |  %s", dv_lbl, cat_lbl),
          subtitle = sprintf("%s  |  %s  |  Adj R²=%.4f  |  OOS RMSE=%.4f",
                             ord_str, method_lbl,
                             if (is.finite(adj_r2_v)) adj_r2_v else NA,
                             if (is.finite(oos_met$rmse)) oos_met$rmse else NA),
          theme = theme(
            plot.title    = element_text(face="bold", size=14),
            plot.subtitle = element_text(size=9, colour="grey40"))
        )
      print(page14)
      message(sprintf("    P14: page added — %s | %s", dv_lbl, cat_lbl))
    }

    grDevices::dev.off()
    message(sprintf("    Saved: %s  (%d pages)", basename(p14_path), nrow(pure_models)))
  }

} else {
  message("    [SKIP] P14 — no pure-ARIMA models found")
}
...
if (nrow(arima_base_all) == 0) {
  ...
  for (sr in names(series_meta_3a)) {
    ...
    hm_pnum <- if (sr == "ficu_count") "P16a" else "P16b"
    hm_path <- file.path(PLOT_DIR,
