...
fit_window_3a <- function(train_dt, test_row, dep_var, feats,
  ...
  if (!is.finite(y_var) || y_var < 1e-6) {
    ...
    cf_all  <- arima_base$coef
    cf_se_v <- tryCatch(sqrt(diag(arima_base$var.coef)), error=function(e) rep(NA_real_, length(cf_all)))
    n_df    <- max(n_obs - length(cf_all), 1L)
    cf_tval <- cf_all / cf_se_v
    cf_pval <- 2 * pt(-abs(cf_tval), df=n_df)
    coef_dt <- data.table(variable=names(cf_all), estimate=as.numeric(cf_all),
                          std_err=as.numeric(cf_se_v), t_stat=as.numeric(cf_tval),
                          p_value=as.numeric(cf_pval), selected=FALSE)
    return(list(ok=TRUE, pred_final=pred_final,
                pred_lo95=pred_lo95, pred_hi95=pred_hi95,
  ...
  if (length(sig_vars_gated) < length(sig_vars)) {
    ...
    if (length(sig_vars_gated) == 0L) {
      ...
      # DO NOT call refit_fn(character(0)) — that produces a mismatched
      # model object that makes forecast() fail.
      arimax_final <- arima_base
      sig_vars     <- character(0)
      ...
      if (!is.null(res_gated$fit)) {
        ...
        # Refit with reduced xreg failed — fall back to pure ARIMA
        arimax_final <- arima_base
        sig_vars     <- character(0)
