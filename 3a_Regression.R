...
fit_window_3a <- function(train_dt, test_row, dep_var, feats,
  ...
  if (sparse_mode) {
    ...
    cf_all  <- arima_base$coef
    cf_se_v <- sqrt(diag(arima_base$var.coef))
    cf_tval <- cf_all / cf_se_v
    n_df    <- max(n_obs - length(cf_all), 1L)
    cf_pval <- 2 * pt(-abs(cf_tval), df=n_df)
    coef_dt <- data.table(variable=names(cf_all),
    ...
    resid_v <- as.numeric(residuals(arima_base))
    ...
    k_p     <- length(cf_all)
    adj_r2  <- if (!is.na(r2_raw) && isTRUE(n_obs > k_p+1L))
                 1 - (1-r2_raw)*(n_obs-1L)/(n_obs-k_p-1L) else NA_real_
  ...
  check_exit_signs <- function(fit, vars, n_obs) {
    ...
    for (ev in intersect(vars, names(EXIT_SIGN_PRIOR))) {
      ...
      # Drop ONLY when BOTH conditions are true:
      #   wrong sign  → coefficient contradicts economic theory
      #   insignificant → no statistical evidence it belongs
      # If wrong sign but significant: keep (data may genuinely show this)
      # If right sign but insignificant: keep (TSCV already screened it)
      if (wrong_sign && insig) {
        message(sprintf("        [EXIT GATE] dropped '%s': est=%.4f  p=%.4f  (wrong sign + insignificant)",
                        ev, est, pval))
        drop_v <- c(drop_v, ev)
      } else if (wrong_sign && !insig) {
        message(sprintf("        [EXIT GATE] KEPT '%s': est=%.4f  p=%.4f  (wrong sign but significant — data-driven)",
                        ev, est, pval))
      } else if (!wrong_sign && insig) {
        message(sprintf("        [EXIT GATE] KEPT '%s': est=%.4f  p=%.4f  (correct sign, n.s. — TSCV screened)",
                        ev, est, pval))
      }
    }
    setdiff(vars, drop_v)
  ...
  if (length(sig_vars_gated) < length(sig_vars)) {
    ...
    if (!is.null(res_gated$fit)) {
      ...
      message("        [EXIT GATE] refit failed — keeping original fit")
      sig_vars <- sig_vars_gated   # at least remove the var from forecast xreg
    }
  } else {
