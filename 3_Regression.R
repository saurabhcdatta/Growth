...
fit_window <- function(train_dt, test_row,
  ...
  # yearqtr arithmetic: convert date to year + quarter fraction
  train_dates <- train_dt$date
  start_yr  <- as.numeric(format(min(train_dates), "%Y"))
  start_q   <- as.integer(substr(as.character(min(train_dates)),
                                  nchar(as.character(min(train_dates))),
                                  nchar(as.character(min(train_dates)))))
  ...
  # ---- Build xreg test vector ----
  build_xreg_test <- function(vars) {
    if (length(vars) == 0L) return(NULL)
    mat <- X_test_aligned[, intersect(vars, colnames(X_test_aligned)),
                           drop = FALSE]
    if (ncol(mat) == 0L) return(NULL)
    mat
  }
  xreg_test <- build_xreg_test(sig_vars)
