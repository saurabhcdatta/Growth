...
#
# v4.2 changes (compatible with macro_v4_frb.R + part1_v4):
#   • USE_MACRO_VARS toggle — enable/disable all FRB macro vars globally
#   • USE_CU_VARS toggle    — enable/disable all CU-level panel vars globally
#   • MAX_XREG_VARS         — hard cap on regressors entering auto.arima()
#   • OVERFIT_ADJ_R2_MAX    — triggers strict elimination if Adj-R² too high
#   • FORECAST_YOY_CAP      — clamps per-quarter YoY% to prevent explosion
#   • MAX_LEVEL_DRIFT_PCT   — fallback to pure ARIMA if level drifts too far
...
# ── Macro variable toggle ─────────────────────────────────────
# TRUE  : all FRB macro variables (yoy/qoq/lag/rmean transforms
#         of the 119 FRB Baseline series) are included as LASSO
#         candidates alongside CU-level financial variables.
#         Produces richest feature set; LASSO + ARIMAX elimination
#         then selects the most predictive subset.
# FALSE : macro variables excluded entirely. Only CU-level panel
#         features (deposits, loans, members, M&A rates, asset
#         transforms, seasonal dummies, etc.) enter the model.
#         Faster, more parsimonious, interpretable without macro.
USE_MACRO_VARS <- TRUE

# ── CU-level variable toggle ──────────────────────────────────
# TRUE  : all CU-level panel variables (deposits, loans, members,
#         assets, M&A rates, acquisition rates, expense ratios,
#         rolling means/SDs, lags, acceleration, cyclical devs,
#         regime dummies, share features, seasonal dummies, etc.)
#         are included as LASSO candidates alongside macro vars.
# FALSE : CU-level variables excluded entirely. Only FRB macro
#         variables enter the model. Useful for a pure macro-only
#         baseline or to assess macro contribution in isolation.
#         NOTE: requires USE_MACRO_VARS = TRUE to have any features.
USE_CU_VARS <- TRUE

# ── Anti-overfitting & forecast stability controls ────────────
#
# MAX_XREG_VARS
#   Hard cap on the number of xreg variables passed to auto.arima().
#   Even after LASSO selects e.g. 30 vars, only the top MAX_XREG_VARS
#   by absolute LASSO coefficient are used. Prevents ARIMA(0,0,0)+30
#   regressors (= OLS with perfect in-sample fit but no dynamics).
#   Rule of thumb: n_train/5 is reasonable; 10-15 is a safe ceiling.
MAX_XREG_VARS <- 12L

# OVERFIT_ADJ_R2_MAX
#   If the final model's in-sample adjusted R² exceeds this threshold
#   AND the ARIMA order is (0,0,0) — meaning no time-series dynamics
#   and the model is essentially OLS — the model is flagged as
#   potentially overfit and a stricter SIG_LEVEL is applied to force
#   further variable reduction via one additional elimination pass.
OVERFIT_ADJ_R2_MAX <- 0.97

# FORECAST_YOY_CAP
#   Per-quarter YoY% forecast is clamped to ±FORECAST_YOY_CAP percent.
#   CU count YoY% rarely exceeds ±20% historically; assets rarely ±30%.
#   This prevents a single explosive quarter from compounding into an
#   impossible level trajectory (e.g. 10B → 100B in 5 years).
FORECAST_YOY_CAP <- 20.0   # ±20% per year cap on YoY% forecasts

# MAX_LEVEL_DRIFT_PCT
#   After back-transforming the multi-step ARIMAX forecast to levels,
#   if the forecast level drifts more than MAX_LEVEL_DRIFT_PCT% above
#   or below the last observed level by the 5-year horizon, the xreg
#   component is suppressed and a pure ARIMA forecast (mean-reverting,
#   no macro regressors) is used instead for that category × target.
#   Set to Inf to disable.
MAX_LEVEL_DRIFT_PCT <- 60.0  # allow up to 60% total drift over 5 years
...
# ── Apply USE_MACRO_VARS toggle ───────────────────────────────
# Macro columns are identified as those whose names contain one of
# the ~119 FRB VAR_MAP short-name roots (uppercase letters) or are
# explicitly listed in the macro-only part of FEAT_PAT above.
# We identify them by matching against the known FRB column roots:
# any feature whose base name (before _yoy/_lag/_rmean etc.) is
# ALL-CAPS or matches known FRB series prefixes is macro-origin.
#
# Safe heuristic: FRB variable names in macro_v4_frb.R are mapped
# to short names like "GDP", "UNRATE", "FEDFUNDS", "GS10", "BAA",
# "PCND", "JCXFE", "SSWAP3Y", "YCCCADJS", etc. — they all appear
# in FEATS_ALL as yoy_GDP, yoy_UNRATE, qoq_SSWAP3Y, GS10_lag4 etc.
# CU-level features use lowercase names (yoy_dep_tot, yoy_members…).
#
# Pattern: column name contains an uppercase run of ≥2 consecutive
# capitals (e.g. GDP, UNRATE, GS10, BAA, PCND, JCXFE, YSCSGE…).
# This reliably separates FRB macro series from CU financial vars.
MACRO_COLS_PAT <- "[A-Z]{2,}"   # ≥2 consecutive uppercase letters

if (!USE_MACRO_VARS) {
  macro_feats <- grep(MACRO_COLS_PAT, FEATS_ALL, value = TRUE, perl = TRUE)
  # Also exclude the explicitly-named macro level vars in FEAT_PAT
  explicit_macro <- grep(
    paste("^fedfunds", "^gs3m", "^gs10", "^yield_curve",
          "^hy_spread", "^baa_spread", "^credit_tightness",
          "^unrate", "^cpi_yoy", "^real_rate", "^spread_2s10s",
          "^mortgage30", "^oil_qoq_pct", "^oil_shock",
          "^fomc_regime", "^hike_run", "^spread_2s10s",
          sep = "|"),
    FEATS_ALL, value = TRUE, perl = TRUE)
  macro_feats  <- unique(c(macro_feats, explicit_macro))
  FEATS_ALL    <- setdiff(FEATS_ALL, macro_feats)
  message(sprintf("    [USE_MACRO_VARS=FALSE] Dropped %d macro features. Remaining: %d",
                  length(macro_feats), length(FEATS_ALL)))
} else {
  message(sprintf("    [USE_MACRO_VARS=TRUE]  All macro features included. Total: %d",
                  length(FEATS_ALL)))
}

# ── Apply USE_CU_VARS toggle ──────────────────────────────────
# CU-level columns are identified as the complement of macro columns:
# any feature in FEATS_ALL that does NOT contain ≥2 consecutive
# uppercase letters (the macro heuristic) AND is not an explicitly-
# named macro level variable is treated as a CU-level feature.
# This covers: yoy_dep_tot, yoy_members, qoq_assets_tot, merger_rate,
# acquisition_rate, share_*, q1/q2/q3, regime_*, time_idx, etc.
if (!USE_CU_VARS) {
  if (!USE_MACRO_VARS)
    warning("[CONFIG] Both USE_MACRO_VARS and USE_CU_VARS are FALSE — ",
            "no features will remain. Setting USE_CU_VARS = TRUE.")

  # CU features = everything that is NOT flagged as macro
  macro_pat_check <- "[A-Z]{2,}"
  explicit_macro_pat <- paste(
    "^fedfunds", "^gs3m", "^gs10", "^yield_curve",
    "^hy_spread", "^baa_spread", "^credit_tightness",
    "^unrate", "^cpi_yoy", "^real_rate", "^spread_2s10s",
    "^mortgage30", "^oil_qoq_pct", "^oil_shock",
    "^fomc_regime", "^hike_run",
    sep = "|")
  is_macro <- grepl(macro_pat_check, FEATS_ALL, perl = TRUE) |
              grepl(explicit_macro_pat, FEATS_ALL, perl = TRUE)
  cu_feats  <- FEATS_ALL[!is_macro]
  FEATS_ALL <- setdiff(FEATS_ALL, cu_feats)
  message(sprintf("    [USE_CU_VARS=FALSE] Dropped %d CU-level features. Remaining: %d",
                  length(cu_feats), length(FEATS_ALL)))
} else {
  message(sprintf("    [USE_CU_VARS=TRUE]  All CU-level features included. Total: %d",
                  length(FEATS_ALL)))
}

# ── Guard: warn if feature set is empty after toggling ───────
if (length(FEATS_ALL) == 0L)
  stop("[CONFIG ERROR] No features remain after applying USE_MACRO_VARS / ",
       "USE_CU_VARS toggles. Enable at least one.")
...
fit_window <- function(train_dt, test_row,
  ...
  build_xreg <- function(vars) {
    ...
    mat  <- mat[, ok_v, drop = FALSE]
    if (ncol(mat) == 0L) return(NULL)
    mat
  }

  # ── Cap xreg candidates at MAX_XREG_VARS ─────────────────────
  # Too many regressors relative to n_obs causes ARIMA(0,0,0)+many-xreg
  # which is OLS in disguise: perfect in-sample fit, explosive forecasts.
  # Keep only the top MAX_XREG_VARS by absolute LASSO coefficient magnitude.
  if (length(xreg_vars) > MAX_XREG_VARS) {
    lasso_abs_all <- abs(lasso_coef[xreg_vars, 1L])
    xreg_vars     <- names(sort(lasso_abs_all, decreasing = TRUE))[
                       seq_len(MAX_XREG_VARS)]
    message(sprintf("        [OVERFIT GUARD] xreg capped at %d vars (was %d)",
                    MAX_XREG_VARS, length(lasso_abs_all)))
  }
  ...
  #
  # Helper: named empty numeric vector
  named_numeric <- function(n) setNames(numeric(n), character(n))
  ...
  # Strategy: drop the SINGLE least-significant xreg variable per iteration
  # (highest p-value above sig_level). This is classic backward stepwise —
  # more conservative than dropping all insignificant vars at once, and ensures
  # p-values are recomputed after each removal (collinearity shifts them).
  # Quarterly dummies (q1/q2/q3) are always protected.
  # NOTE: sign_ok filter NOT applied here — ARIMAX xreg coefs are partial effects
  # conditional on ARIMA dynamics; economic sign priors don't reliably apply.
  ...
  # Returns list(fit, vars) where vars = colnames actually used by the model.
  # Uses method="CSS-ML" → "ML" fallback chain for numerical stability.
  refit_arima <- function(vars) {
    ...
    # Try CSS-ML first (faster, more stable with many xreg)
    fit <- tryCatch(
      forecast::Arima(y_ts,
        order    = c(p_ord, d_ord, q_ord),
        seasonal = list(order = if (has_seasonal)
                          c(P_ord, D_ord, Q_ord) else c(0L, 0L, 0L),
                        period = 4L),
        xreg     = m,
        method   = "CSS-ML"),
      error = function(e) NULL)
    ...
    list(fit = fit, vars = get_xreg_names_from_fit(fit))
  }

  # Helper: compute p-values for xreg coefs in a fitted Arima object
  xreg_pvals <- function(fit) {
    xv   <- get_xreg_names_from_fit(fit)
    if (length(xv) == 0L) return(named_numeric(0))
    cf   <- fit$coef[xv]
    se   <- sqrt(diag(fit$var.coef))[match(xv, rownames(fit$var.coef))]
    tval <- cf / se
    n_df <- max(length(y_train_w) - length(fit$coef), 1L)
    pv   <- 2 * pt(-abs(tval), df = n_df)
    setNames(pv, xv)
  }

  arimax_final    <- arimax_fit
  MAX_ITER_ARIMAX <- 30L   # more iterations since we drop 1 var per pass
  iter            <- 0L
  q_protect       <- intersect(c("q1","q2","q3"), x_train_cols)

  n_xreg_init <- length(actual_fit_vars)
  if (length(actual_fit_vars) > 0L) {
    repeat {
      ...
      pv <- xreg_pvals(arimax_final)

      # Candidates for removal: not a quarterly dummy AND p >= sig_level
      removable <- setdiff(names(pv), q_protect)
      insig     <- removable[pv[removable] >= sig_level]

      if (length(insig) == 0L) break   # all xreg significant — stable

      # Drop the single worst variable (highest p-value) this iteration
      drop_var  <- insig[which.max(pv[insig])]
      keep_vars <- setdiff(xreg_in_model, drop_var)
      ...
      if (is.null(res_refit$fit)) {
        # Refit failed with this set — try removing the next-worst var instead
        # (Some variable combinations cause numerical issues in Arima())
        insig_remaining <- insig[insig != drop_var]
        if (length(insig_remaining) == 0L) break
        drop_var2  <- insig_remaining[which.max(pv[insig_remaining])]
        keep_vars2 <- setdiff(xreg_in_model, drop_var2)
        keep_vars2 <- intersect(keep_vars2, x_train_cols)
        res_refit2 <- refit_arima(keep_vars2)
        if (is.null(res_refit2$fit)) break   # both failed — stop elimination
        res_refit <- res_refit2
      }
      arimax_final    <- res_refit$fit
      actual_fit_vars <- res_refit$vars
    }
  }
  ...
  if (iter > 0L)
    message(sprintf("        [ARIMAX ELIM] %d iterations: %d xreg → %d final",
                    iter, n_xreg_init, length(sig_vars)))

  # ── Overfit detection: ARIMA(0,0,0) + high adj-R² ─────────────
  # If the model has no AR/MA dynamics (pure regression) AND adj-R²
  # is very high, it is memorising training noise. Apply a stricter
  # significance threshold (half of sig_level) for one additional
  # elimination pass to force further variable reduction.
  # Use as.integer() to strip names before comparison — named integers
  # from arimaorder() can produce NA in all() if any element has NA name.
  is_arima000 <- isTRUE(all(c(as.integer(p_ord), as.integer(d_ord),
                               as.integer(q_ord), as.integer(P_ord),
                               as.integer(D_ord), as.integer(Q_ord)) == 0L))
  resid_tmp   <- as.numeric(residuals(arimax_final))
  n_tmp       <- length(y_train_w)
  k_tmp       <- length(arimax_final$coef)
  ss_res_tmp  <- sum(resid_tmp^2, na.rm = TRUE)
  ss_tot_tmp  <- sum((y_train_w - mean(y_train_w, na.rm=TRUE))^2, na.rm=TRUE)
  r2_tmp      <- if (isTRUE(ss_tot_tmp > 0)) 1 - ss_res_tmp / ss_tot_tmp else NA_real_
  adj_r2_tmp  <- if (!is.na(r2_tmp) && isTRUE(n_tmp > k_tmp + 1L))
                   1 - (1 - r2_tmp) * (n_tmp - 1L) / (n_tmp - k_tmp - 1L)
                 else NA_real_
  adj_r2_tmp  <- if (is.finite(adj_r2_tmp)) adj_r2_tmp else NA_real_

  if (isTRUE(is_arima000) && isTRUE(!is.na(adj_r2_tmp)) &&
      isTRUE(adj_r2_tmp > OVERFIT_ADJ_R2_MAX) &&
      length(sig_vars) > 2L) {
    message(sprintf(
      "        [OVERFIT GUARD] ARIMA(0,0,0) + Adj-R²=%.3f > %.2f — stricter elimination pass",
      adj_r2_tmp, OVERFIT_ADJ_R2_MAX))
    strict_level <- sig_level / 2   # e.g. 0.05 instead of 0.10
    iter2 <- 0L
    repeat {
      iter2 <- iter2 + 1L
      if (iter2 > MAX_ITER_ARIMAX) break
      xreg_in_m2 <- get_xreg_names_from_fit(arimax_final)
      if (length(xreg_in_m2) == 0L) { actual_fit_vars <- character(0); break }
      pv2       <- xreg_pvals(arimax_final)
      removable2 <- setdiff(names(pv2), q_protect)
      insig2     <- removable2[pv2[removable2] >= strict_level]
      if (length(insig2) == 0L) break
      drop2      <- insig2[which.max(pv2[insig2])]
      keep2      <- intersect(setdiff(xreg_in_m2, drop2), x_train_cols)
      if (length(keep2) == 0L) { actual_fit_vars <- character(0); break }
      rr2 <- refit_arima(keep2)
      if (is.null(rr2$fit)) break
      arimax_final    <- rr2$fit
      actual_fit_vars <- rr2$vars
    }
    sig_vars <- actual_fit_vars
    message(sprintf("        [OVERFIT GUARD] After strict pass: %d xreg vars remain",
                    length(sig_vars)))
  }
  ...
  # ── Layer 4: YoY% cap — clamp to ±FORECAST_YOY_CAP ──────────
  # Derive a data-driven cap from historical training range,
  # bounded by the global FORECAST_YOY_CAP setting.
  hist_yoy_range <- max(abs(y_train_w), na.rm = TRUE)
  dynamic_cap    <- min(FORECAST_YOY_CAP,
                        max(hist_yoy_range * 1.25, 5.0))  # 25% headroom above history
  if (!is.na(pred_final) && abs(pred_final) > dynamic_cap) {
    message(sprintf("        [YOY CAP] pred=%.2f clamped to ±%.2f", pred_final, dynamic_cap))
    pred_final <- sign(pred_final) * dynamic_cap
  }
  pred_lo95 <- pmax(pred_lo95, -dynamic_cap * 1.5)  # CI allowed wider than point
  pred_hi95 <- pmin(pred_hi95,  dynamic_cap * 1.5)
...
for (key in names(all_ols_fits)) {
  ...
  # ── Level drift guard ─────────────────────────────────────────
  # After generating the multi-step forecast, check whether the
  # implied level trajectory drifts beyond MAX_LEVEL_DRIFT_PCT%
  # of the last observed level by end of horizon. If so, suppress
  # xreg entirely and reforecast with pure ARIMA (mean-reverting).
  if (is.finite(MAX_LEVEL_DRIFT_PCT)) {
    fc_mean_tmp <- pmax(pmin(as.numeric(fc_arimax$mean),
                              FORECAST_YOY_CAP), -FORECAST_YOY_CAP)
    # Compound the YoY% to approximate level drift
    # Each quarter contributes (1 + yoy/400) to the quarterly growth rate
    compound_factor <- prod(1 + fc_mean_tmp / 400, na.rm = TRUE)
    total_drift_pct <- abs(compound_factor - 1) * 100

    if (total_drift_pct > MAX_LEVEL_DRIFT_PCT && has_xreg) {
      message(sprintf(
        "    [DRIFT GUARD] %s: %.1f%% drift > %.1f%% cap — falling back to pure ARIMA",
        key, total_drift_pct, MAX_LEVEL_DRIFT_PCT))
      # Refit pure ARIMA on the same training series
      pure_fc_fit <- tryCatch(
        forecast::auto.arima(
          ts(tail(qtrly[cat_label == cat_lbl, get(dv)], obj$n_train),
             frequency = 4L),
          stepwise = TRUE, approximation = TRUE,
          max.p = 3L, max.q = 2L, max.P = 1L, max.Q = 1L),
        error = function(e) NULL)
      if (!is.null(pure_fc_fit)) {
        fc_arimax <- tryCatch(
          forecast::forecast(pure_fc_fit, h = h_steps, level = 95),
          error = function(e) NULL)
        if (is.null(fc_arimax)) {
          message(sprintf("    [DRIFT GUARD] Pure ARIMA forecast also failed for %s", key))
          next
        }
        message(sprintf("    [DRIFT GUARD] Pure ARIMA fallback succeeded for %s", key))
      }
    }
  }
  ...
  # ── Clamp YoY% forecasts to ±FORECAST_YOY_CAP ────────────────
  # Prevents a single extreme macro projection from compounding
  # into an impossible level trajectory over the 5-year horizon.
  fc_mean <- pmax(pmin(fc_mean, FORECAST_YOY_CAP), -FORECAST_YOY_CAP)
  fc_lo95 <- pmax(pmin(fc_lo95, FORECAST_YOY_CAP), -FORECAST_YOY_CAP)
  fc_hi95 <- pmax(pmin(fc_hi95, FORECAST_YOY_CAP), -FORECAST_YOY_CAP)

  # ── Layer 4b: clamp future YoY% forecasts ────────────────────
  # Same cap as rolling window — prevents individual extreme quarters
  # from compounding into impossible 5-year level trajectories.
  hist_yoy_fut <- qtrly[cat_label == cat_lbl, get(dv)]
  hist_cap_fut <- min(FORECAST_YOY_CAP,
                      max(abs(hist_yoy_fut), na.rm = TRUE) * 1.25)
  fc_mean <- pmax(pmin(fc_mean, hist_cap_fut), -hist_cap_fut)
  fc_lo95 <- pmax(fc_lo95, -hist_cap_fut * 1.5)
  fc_hi95 <- pmin(fc_hi95,  hist_cap_fut * 1.5)

  # ── Layer 5: level drift guard ───────────────────────────────
  # Back-transform YoY% to levels and check if the 5-year endpoint
  # drifts beyond MAX_LEVEL_DRIFT_PCT from the last observed level.
  # If it does, suppress xreg and re-run as pure ARIMA.
  if (!is.infinite(MAX_LEVEL_DRIFT_PCT) && has_xreg) {
    # Quick back-transform of the point forecast to check final level
    anchor_check <- switch(dv,
      yoy_ficu_pct   = last_known$ficu_count,
      yoy_fiscu_pct  = last_known$fiscu_count,
      yoy_assets_pct = last_known$assets_tot,
      NA_real_)
    if (!is.na(anchor_check) && anchor_check > 0) {
      # Compound the YoY% forecast to get approximate level at end of horizon
      level_approx <- anchor_check
      for (yoy_q in fc_mean) {
        level_approx <- level_approx * (1 + yoy_q / 100)
      }
      drift_pct <- abs(level_approx - anchor_check) / anchor_check * 100
      if (drift_pct > MAX_LEVEL_DRIFT_PCT) {
        message(sprintf(
          "    [DRIFT GUARD] %s|%s: %.1f%% drift > %.1f%% limit — switching to pure ARIMA",
          dv, cat_lbl, drift_pct, MAX_LEVEL_DRIFT_PCT))
        # Re-run forecast with pure ARIMA (no xreg = mean-reverting)
        # Reconstruct training series for this cat/dv from qtrly
        cat_y_raw  <- qtrly[cat_label == cat_lbl & date <= obj$date_end,
                             get(dv)]
        cat_dates2 <- sort(qtrly[cat_label == cat_lbl & date <= obj$date_end, date])
        cat_y_ts   <- if (length(cat_y_raw) >= 4L) {
          start_yr2 <- as.integer(format(zoo::as.yearqtr(min(cat_dates2)), "%Y"))
          start_q2  <- as.integer(format(zoo::as.yearqtr(min(cat_dates2)), "%q"))
          ts(cat_y_raw, frequency = 4L, start = c(start_yr2, start_q2))
        } else NULL
        ao_fc <- forecast::arimaorder(fit)
        pure_fit_fc <- if (!is.null(cat_y_ts)) tryCatch(
          forecast::Arima(cat_y_ts,
            order    = c(ao_fc["p"], ao_fc["d"], ao_fc["q"]),
            seasonal = list(order  = c(ao_fc["P"], ao_fc["D"], ao_fc["Q"]),
                            period = 4L),
            method   = "CSS-ML"),
          error = function(e) NULL) else NULL
        if (!is.null(pure_fit_fc)) {
          fc_pure <- tryCatch(
            forecast::forecast(pure_fit_fc, h = h_steps, level = 95),
            error = function(e) NULL)
          if (!is.null(fc_pure)) {
            fc_mean  <- pmax(pmin(as.numeric(fc_pure$mean), hist_cap_fut), -hist_cap_fut)
            fc_lo95  <- as.numeric(fc_pure$lower[, 1L])
            fc_hi95  <- as.numeric(fc_pure$upper[, 1L])
            message(sprintf("    [DRIFT GUARD] Pure ARIMA substituted for %s|%s", dv, cat_lbl))
          }
        }
      }
    }
