...
#
# Each sheet contains:
#   Columns A-B : Quarter | Row type (Historical / Forecast)
#   Column  C   : FICU Count      (actual for hist; predicted for fcst)
#   Column  D   : FICU Count Lo95 (NA for hist; 95% PI lower for fcst)
#   Column  E   : FICU Count Hi95 (NA for hist; 95% PI upper for fcst)
#   Column  F   : FISCU Count
#   Column  G   : FISCU Count Lo95
#   Column  H   : FISCU Count Hi95
#   Column  I   : Total Assets ($000s)
#   Column  J   : Total Assets Lo95
#   Column  K   : Total Assets Hi95
#   Column  L   : Method (OLS / LASSO / Historical)
#
# Formatting:
#   • Header row: dark navy fill, white bold text
#   • Historical rows: white background
#   • Forecast rows: amber (#FFF3CD) background
#   • 95% CI columns: light blue (#DEEBF7) background
#   • Number formats: comma for counts, $ millions for assets
#   • Column widths auto-fitted
#   • Sheet names = short category labels
#   • First sheet = README with model notes
# ════════════════════════════════════════════════════════════
message("\n[9] Excel export — one tab per asset category...")
...
hist_style <- createStyle(
  fgFill = "#FFFFFF",
  border = "Bottom",
  borderColour = "#E0E0E0"
)

fc_style <- createStyle(
  fgFill = "#FFF3CD",
  border = "Bottom",
  borderColour = "#E0E0E0"
)

ci_hist_style <- createStyle(
  fgFill = "#FFFFFF",
  fontColour = "#AAAAAA",   # greyed — NAs for history
  border = "Bottom",
  borderColour = "#E0E0E0"
)

ci_fc_style <- createStyle(
  fgFill = "#DEEBF7",
  border = "Bottom",
  borderColour = "#E0E0E0"
)

count_fmt   <- createStyle(numFmt = "#,##0")
count_ci_fmt <- createStyle(numFmt = "#,##0", fgFill = "#DEEBF7",
                             border = "Bottom", borderColour = "#E0E0E0")
dollar_fmt  <- createStyle(numFmt = '#,##0.0"M"')
dollar_ci_fmt <- createStyle(numFmt = '#,##0.0"M"', fgFill = "#DEEBF7",
                              border = "Bottom", borderColour = "#E0E0E0")

# ── Short sheet names for tab labels ──────────────────────
cat_sheet_names <- c(
  "1_Less_10M"   = "< $10M",
  "2_10M_50M"    = "$10M-50M",
  "3_50M_100M"   = "$50M-100M",
...
# ── README sheet ──────────────────────────────────────────
addWorksheet(wb, "README", tabColour = "#1A3A5C")
readme_rows <- data.frame(Field = c(
  "Report title",
  "Generated",
  "Model",
  "Dep vars (forecast space)",
  "Level vars (this workbook)",
  "Training period",
  ...
  sprintf("p < %.2f; variables must also pass sign-consistency check", SIG_LEVEL),
  "95% prediction intervals from OLS predict(interval='prediction'); LASSO rows use ±1.96×train RMSE",
  "Out-of-sample forecast quarters",
  "95% PI lower and upper bounds",
  "OLS = post-LASSO OLS used; LASSO = fell back to LASSO; Historical = actual data",
  "Federally Insured Credit Union",
  "Federally Insured State-Chartered Credit Union",
  "Sum of total assets within category, in thousands of dollars ($000s)"
))

writeDataTable(wb, "README", readme_rows, startRow = 2, startCol = 1,
               tableStyle = "TableStyleMedium9",
               tableName  = "README_tbl")
...
setColWidths(wb, "README", cols = 1:2, widths = c(32, 80))
...
for (cat in cats_xl) {
  ...
  sheet_nm <- cat_sheet_names[cat]
  if (is.na(sheet_nm) || nchar(sheet_nm) == 0) sheet_nm <- cat
  addWorksheet(wb, sheet_nm, tabColour = "#2171B5")

  # ── Pull historical data (full_levels for this cat) ──
  ...
  # ── Pull forecast data for each series ──────────────────
  ...
  # All three series should share the same date set — use union of dates
  fc_dates <- sort(unique(c(fc_cat_ficu$date, fc_cat_fiscu$date,
                             fc_cat_assets$date)))

  if (length(fc_dates) > 0) {

    get_fc_vals <- function(fc_dt, date_vec) {
      # Returns named list: pred, lo, hi, method
      idx <- match(date_vec, fc_dt$date)
      list(
        pred   = fc_dt$pred_level[idx],
        lo95   = if ("pred_lo95" %in% names(fc_dt)) fc_dt$pred_lo95[idx] else rep(NA_real_, length(date_vec)),
        hi95   = if ("pred_hi95" %in% names(fc_dt)) fc_dt$pred_hi95[idx] else rep(NA_real_, length(date_vec)),
        method = fc_dt$method_used[idx]
      )
    }

    ficu_v  <- get_fc_vals(fc_cat_ficu,  fc_dates)
    fiscu_v <- get_fc_vals(fc_cat_fiscu, fc_dates)
    asset_v <- get_fc_vals(fc_cat_assets,fc_dates)

    # Method: prefer OLS, then LASSO, then whatever is available
    method_vec <- ifelse(!is.na(ficu_v$method),  ficu_v$method,
                  ifelse(!is.na(fiscu_v$method), fiscu_v$method,
                         asset_v$method))

    fc_rows_xl <- data.table(
      Quarter      = as.character(fc_dates),
      Row_Type     = "Forecast",
      FICU_Count   = round(ficu_v$pred),
      FICU_Lo95    = round(ficu_v$lo95),
      FICU_Hi95    = round(ficu_v$hi95),
      FISCU_Count  = round(fiscu_v$pred),
      FISCU_Lo95   = round(fiscu_v$lo95),
      FISCU_Hi95   = round(fiscu_v$hi95),
      Assets_000s  = round(asset_v$pred),
      Assets_Lo95  = round(asset_v$lo95),
      Assets_Hi95  = round(asset_v$hi95),
      Method       = method_vec
    )

    # Remove fc rows that duplicate historical (shouldn't happen but guard)
    fc_rows_xl <- fc_rows_xl[!Quarter %in% hist_rows$Quarter]
  ...
  setorderv(all_rows, "Quarter")

  # ── Sheet title row ───────────────────────────────────────
  title_txt <- sprintf(
    "Credit Union Forecast — %s Asset Category  |  Generated: %s",
    sheet_nm, format(Sys.time(), "%Y-%m-%d"))
  writeData(wb, sheet_nm, title_txt, startRow = 1, startCol = 1)
  addStyle(wb, sheet_nm,
           createStyle(fontSize = 12, textDecoration = "bold",
                       fontColour = "#1A3A5C"),
           rows = 1, cols = 1)
  mergeCells(wb, sheet_nm, cols = 1:12, rows = 1)
  ...
  col_headers <- data.frame(
    ...
    Assets_000s   = "Total Assets\n($000s)",
    Assets_Lo95   = "Assets Lo 95%\n($000s)",
    Assets_Hi95   = "Assets Hi 95%\n($000s)",
    Method        = "Method"
  )
  writeData(wb, sheet_nm, col_headers, startRow = 2, startCol = 1,
            colNames = FALSE)
  addStyle(wb, sheet_nm, hdr_style,
           rows = 2, cols = 1:12, gridExpand = TRUE)
  setRowHeights(wb, sheet_nm, rows = 2, heights = 36)
  ...
  # Write all data
  writeData(wb, sheet_nm, all_rows,
            startRow = data_start_row, startCol = 1,
            colNames = FALSE)

  # Apply row styles
  for (i in seq_len(n_data)) {
    r        <- data_start_row + i - 1L
    is_fc    <- all_rows$Row_Type[i] == "Forecast"
    base_sty <- if (is_fc) fc_style  else hist_style
    ci_sty   <- if (is_fc) ci_fc_style else ci_hist_style
    cnt_sty  <- if (is_fc) {
      createStyle(fgFill="#FFF3CD", numFmt="#,##0",
                  border="Bottom", borderColour="#E0E0E0")
    } else {
      createStyle(fgFill="#FFFFFF", numFmt="#,##0",
                  border="Bottom", borderColour="#E0E0E0")
    }
    cnt_ci_sty <- createStyle(fgFill="#DEEBF7", numFmt="#,##0",
                               border="Bottom", borderColour="#E0E0E0")
    # Assets in $M display (divide by 1000 for display)
    # Actually keep in $000s but format with comma + "K" suffix
    ast_sty  <- if (is_fc) {
      createStyle(fgFill="#FFF3CD", numFmt='#,##0',
                  border="Bottom", borderColour="#E0E0E0")
    } else {
    ...
    ast_ci_sty <- createStyle(fgFill="#DEEBF7", numFmt='#,##0',
                               border="Bottom", borderColour="#E0E0E0")

    # A: Quarter / B: Type / L: Method — text
    addStyle(wb, sheet_nm, base_sty, rows=r, cols=c(1,2,12))
    # C: FICU Count / F: FISCU Count — number count
    addStyle(wb, sheet_nm, cnt_sty,  rows=r, cols=c(3,6))
    # D,E: FICU CI / G,H: FISCU CI — CI blue
    addStyle(wb, sheet_nm, cnt_ci_sty, rows=r, cols=c(4,5,7,8))
    # I: Assets — number
    addStyle(wb, sheet_nm, ast_sty,  rows=r, cols=9)
    # J,K: Assets CI — CI blue
    addStyle(wb, sheet_nm, ast_ci_sty, rows=r, cols=c(10,11))
  }

  # ── Freeze panes at row 3, col 2 ─────────────────────────
  freezePane(wb, sheet_nm, firstActiveRow = 3, firstActiveCol = 2)

  # ── Column widths ─────────────────────────────────────────
  setColWidths(wb, sheet_nm,
               cols    = 1:12,
               widths  = c(12, 12, 14, 14, 14, 14, 14, 14, 16, 16, 16, 11))

  # ── Conditional formatting: forecast rows are already amber;
  #    add data bars to point-estimate columns for visual cue ──
  # FICU Count data bar
  conditionalFormatting(wb, sheet_nm,
    cols = 3, rows = data_start_row:(data_start_row + n_data - 1),
    style = c("#1F77B4"), type = "databar")
  # Assets data bar
  conditionalFormatting(wb, sheet_nm,
    cols = 9, rows = data_start_row:(data_start_row + n_data - 1),
    style = c("#2CA02C"), type = "databar")

  message(sprintf("    Sheet '%s': %d hist rows + %d forecast rows",
                  sheet_nm,
...
notify("Part 3 DONE",
       sprintf("[%s] %dh%02dm | 21 models | %d forecasts | %d level rows | %d plots | Excel: %s",
               if (DEBUG_MODE) "DEBUG" else "PROD",
