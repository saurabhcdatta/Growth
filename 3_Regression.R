    ...
    if (length(ols_feats) >= 1L) {
      ...
      if (!is.null(ols_fit)) {
        ...
        if (length(sig_vars) >= 1L) {
          # FIX: intersect with colnames(X_train) before entering loop.
          # sig_vars comes from LASSO coefficients but prep_X() may have
          # dropped some columns via correlation/missingness filtering,
          # causing "subscript out of bounds" on X_train[, current_vars].
          x_train_cols <- colnames(X_train)
          current_vars <- intersect(sig_vars, x_train_cols)
          repeat {
            iter <- iter + 1L
            if (length(current_vars) == 0L || iter > MAX_ITER) break

            # Re-guard every iteration: keep_vars may contain names from
            # lm() rownames that aren't valid X_train columns (e.g. after
            # alias removal or the 6_1B_10B category that triggers this bug)
            current_vars <- intersect(current_vars, x_train_cols)
            ...
            # Guard keep_vars too before next iteration
            ...
            if (length(keep_vars) == 0L) {
              ...
              q_protected <- intersect(q_protected, x_train_cols)
...
for (key in names(all_ols_fits)) {
  ...
  last_known <- list(
    ...
    fiscu_count = {
      ...
      if (length(v)) v else NA_real_},
    assets_tot  = {
      v <- tail(qtrly[cat_label==cat_lbl & !is.na(assets_tot), assets_tot], 1)
      if (length(v)) v else NA_real_}
  )
