...
build_cat_table <- function(cat) {
  ...
  if (length(fc_dates) > 0) {
    gv <- function(fc_dt, dvec) {
      idx <- match(dvec, fc_dt$date)
      list(
        pred   = fc_dt$pred_level[idx],
        lo95   = if ("pred_lo95" %in% names(fc_dt)) fc_dt$pred_lo95[idx]
                 else rep(NA_real_, length(dvec)),
        hi95   = if ("pred_hi95" %in% names(fc_dt)) fc_dt$pred_hi95[idx]
                 else rep(NA_real_, length(dvec)),
        method = fc_dt$method_used[idx]
      )
    }
    fv <- gv(fc_cat_ficu,  fc_dates)
    sv <- gv(fc_cat_fiscu, fc_dates)
    av <- gv(fc_cat_assets,fc_dates)
