...
# ════════════════════════════════════════════════════════════
# 5c. OLS FUTURE FORECAST  (LAST_DATA_DATE+1Q → 2030Q4)
#
# Takes the final rolling-window OLS model for each dep_var ×
# category (stored in all_ols_fits) and projects forward
# quarter-by-quarter to 2030Q4.
#
# Feature matrix per future quarter is sourced from qtrly
# (if Part 1 V3 forward panel is present); any missing feature
# falls back to the training-window column median.
#
# Results are back-transformed to level space with the same
# yoy_to_level / exp() helpers and appended to level_fc with
# method_used = "OLS_FUTURE" so plots can separate OOS eval
# rows from true future rows.
# ════════════════════════════════════════════════════════════
message("\n[5c] OLS future forecast (last data → 2030Q4)...")

OLS_FC_END <- zoo::as.yearqtr("2030 Q4")

# Helper: predict from lm with 95% PI; missing features set to 0
predict_ols_row <- function(fit, feat_row) {
  needed <- names(coef(fit))
  needed <- needed[needed != "(Intercept)"]
  newdf  <- as.data.frame(matrix(0, nrow=1L, ncol=length(needed),
                                  dimnames=list(NULL, needed)))
  for (v in needed) {
    if (v %in% names(feat_row)) {
      val <- suppressWarnings(as.numeric(feat_row[[v]]))
      if (length(val)==1L && !is.na(val) && is.finite(val))
        newdf[1L, v] <- val
    }
  }
  pred_full <- tryCatch(
    predict(fit, newdata=newdf, interval="prediction", level=0.95),
    error=function(e) NULL)
  if (!is.null(pred_full) && is.matrix(pred_full))
    return(list(fit=pred_full[1L,"fit"],
                lo =pred_full[1L,"lwr"],
                hi =pred_full[1L,"upr"]))
  pt    <- tryCatch(as.numeric(predict(fit, newdata=newdf)),
                   error=function(e) NA_real_)
  rmse_f <- sqrt(mean(residuals(fit)^2, na.rm=TRUE))
  list(fit=pt, lo=pt - 1.96*rmse_f, hi=pt + 1.96*rmse_f)
}
...
for (key in names(all_ols_fits)) {
  ...
  # Running level anchors — initialise from last known actuals
  last_known <- list(
    ficu_count  = {
      v <- tail(qtrly[cat_label==cat_lbl & !is.na(ficu_count),  ficu_count],  1)
      if (length(v)) v else NA_real_},
    fiscu_count = {
      v <- tail(qtrly[cat_label==cat_lbl & !is.na(fiscu_count), fiscu_count], 1)
      if (length(v)) v else NA_real_},
    ln_assets   = {
      v <- tail(qtrly[cat_label==cat_lbl & !is.na(ln_assets_tot), ln_assets_tot], 1)
      if (length(v)) v else NA_real_}
  )
  ...
  for (fq in fc_qtrs) {
    ...
    # Build feature row: forward panel row if available, else medians
    fwd_row <- qtrly[cat_label == cat_lbl & date == fq_yqtr]
    if (nrow(fwd_row) == 1L) {
      feat_row <- as.data.frame(fwd_row)[1L, , drop=FALSE]
      # Replace NA/non-finite numerics with training medians
      for (v in intersect(names(feat_row), names(train_meds))) {
        val <- suppressWarnings(as.numeric(feat_row[[v]]))
        if (!is.na(val) && is.finite(val)) next
        if (is.finite(train_meds[[v]])) feat_row[[v]] <- train_meds[[v]]
      }
    } else {
      feat_row <- as.data.frame(as.list(
        lapply(train_meds, function(m) if(is.finite(m)) m else 0)
      ))
    }

    p <- predict_ols_row(fit, feat_row)
    if (is.na(p$fit)) next
    ...
    } else if (dv == "ln_assets_tot") {
      pred_lv  <- exp(p$fit)
      pred_lo  <- exp(p$lo)
      pred_hi  <- exp(p$hi)
      series_id <- "total_assets"
      y_lbl     <- "Total Assets ($000s)"
      last_known$ln_assets <- p$fit
...
if (length(ols_future_rows) > 0L) {
  ...
  message(sprintf("    OLS future rows: %d  (%s → %s)",
                  nrow(ols_future_dt),
                  as.character(min(ols_future_dt$date)),
                  as.character(max(ols_future_dt$date))))
} else {
  message("    [WARN] No OLS future rows — check all_ols_fits and qtrly forward panel")
}
...
for (sr in names(arima_series_meta)) {
  ...
  # ── OLS forecast levels — use the forward forecast rows only
  ...
  ar_hm[, panel := "ARIMA Forecast"]

  if (nrow(ols_hm) == 0 && nrow(ar_hm) == 0) {
    message(sprintf("    [HM SKIP] %s — no data", sr)); next
  }

  # ── Stack and set category order ─────────────────────────
  hm_all <- rbindlist(list(ols_hm, ar_hm), fill=TRUE)
  cat_order <- rev(sort(unique(hm_all$cat_label)))
  hm_all[, cat_f  := factor(cat_label, levels=cat_order)]
  hm_all[, panel_f := factor(panel,
                              levels=c("OLS Forecast","ARIMA Forecast"))]

  # ── Auto text colour: dark fill → white label ─────────────
  val_range <- range(hm_all$value, na.rm=TRUE)
  hm_all[, text_col := ifelse(
    !is.na(value) & (value - val_range[1]) /
      (val_range[2] - val_range[1] + 1e-9) > 0.55,
    "white", "grey20")]

  # ── Colour scale: light blue → dark blue ─────────────────
  fill_scale <- scale_fill_gradient(
    ...
    name     = label,
    labels   = if (is_dollar)
                 function(x) sprintf("$%.1fB", x/1e6)
               else
                 scales::comma
  )

  p_hm <- ggplot(hm_all,
                  aes(x=date_d, y=cat_f, fill=value)) +
    ...
    facet_wrap(~ panel_f, ncol=1, scales="free_x") +
    labs(
      title    = sprintf("%s — OLS vs ARIMA Forecast Heatmap", label),
      subtitle = paste(
        "Each cell = forecasted level value.",
        if (is_dollar) "Assets shown in $B." else "Count values.",
        "Top: OLS rolling forecast.  Bottom: ARIMA forecast."
      ),
      x=NULL, y=NULL
    ) +
    theme_cu() +
  ...
  hm_path <- file.path(PLOT_DIR,
                        sprintf("%s_heatmap_%s_ols_vs_arima.pdf",
                                hm_pnum, sm$short))
  tryCatch({
    grDevices::cairo_pdf(hm_path, width=16, height=10, onefile=TRUE)
    print(p_hm)
    grDevices::dev.off()
