...
fit_window <- function(train_dt, test_row,
  ...
  # Initial ARIMAX fit — with fallback to pure ARIMA if xreg causes issues
  # Common causes of failure: xreg has more cols than n_obs, rank-deficiency,
  # or too few obs for the seasonal model with many regressors.
  arimax_fit <- tryCatch(
    forecast::auto.arima(
      y_ts,
      xreg          = xreg_train,
    ...
    error = function(e) {
      message(sprintf("        [ARIMAX] auto.arima with xreg failed: %s — trying pure ARIMA", e$message))
      NULL
    }
  )

  # Fallback 1: try with fewer xreg (top LASSO vars only, max 5)
  if (is.null(arimax_fit) && length(xreg_vars) > 0L) {
    ...
    if (!is.null(arimax_fit)) xreg_vars <- top_vars
  }

  # Fallback 2: pure ARIMA (no xreg)
  if (is.null(arimax_fit)) {
    ...
    if (!is.null(arimax_fit)) {
      xreg_vars <- character(0)  # no xreg in this fallback
      xreg_train <- NULL
  ...
  # Keep ARIMA order fixed; only drop insignificant xreg vars.
  # t-threshold: corresponds approximately to p < sig_level (2-sided)
  # with df = n_train - (p+q+P+Q) - length(xreg) - 1
  # Use conservative threshold of 1.65 (p≈0.10) as floor.
  t_thresh   <- max(abs(qt(sig_level / 2,
                           df = max(length(y_train_w) - 10L, 5L))), 1.65)

  # Extract ARIMA order for re-fitting
  ...
  # refit_arima() returns a LIST: $fit (Arima object) + $vars (actual cols used).
  # This is critical — the zero-variance filter inside may silently drop columns,
  # so the model object knows fewer xreg cols than sig_vars. We must build the
  # forecast xreg matrix from $vars (what the model actually saw), not sig_vars.
  refit_arima <- function(vars) {
    if (length(vars) == 0L)
      return(list(fit = NULL, vars = character(0)))
    m    <- X_train[, vars, drop = FALSE]
    ok_v <- apply(m, 2L, function(col) var(col, na.rm = TRUE) > 0)
    m    <- m[, ok_v, drop = FALSE]
    actual_vars <- colnames(m)
    xr <- if (ncol(m) == 0L) NULL else m
    fit <- tryCatch(
      forecast::Arima(
        y_ts,
    ...
    list(fit = fit, vars = if (!is.null(fit)) actual_vars else character(0))
  }

  current_xreg_vars <- xreg_vars
  MAX_ITER_ARIMAX   <- 10L
  iter              <- 0L
  # Seed arimax_final with the initial fit; track actual cols it was fit on
  # The initial auto.arima fit used xreg_train whose cols == xreg_vars (after
  # build_xreg filtering). Re-derive actual cols from xreg_train colnames.
  actual_fit_vars   <- if (!is.null(xreg_train)) colnames(xreg_train) else character(0)
  arimax_final      <- arimax_fit

  repeat {
    iter <- iter + 1L
    if (iter > MAX_ITER_ARIMAX || length(current_xreg_vars) == 0L) break

    # Identify xreg coef names from what the model was actually fit on
    cf_all   <- arimax_final$coef
    cf_se    <- sqrt(diag(arimax_final$var.coef))

    # Use actual_fit_vars to intersect — not current_xreg_vars — because the
    # zero-variance filter may have removed some vars silently in the last refit
    xreg_coef_names <- intersect(actual_fit_vars, names(cf_all))
    if (length(xreg_coef_names) == 0L) break

    t_vals    <- cf_all[xreg_coef_names] /
                  cf_se[match(xreg_coef_names, names(cf_se))]
    keep_vars <- xreg_coef_names[abs(t_vals) >= t_thresh]
    ...
    # Always protect quarterly dummies
    q_dummies <- intersect(c("q1","q2","q3"), xreg_coef_names)
    keep_vars <- unique(c(keep_vars, q_dummies))
    keep_vars <- intersect(keep_vars, x_train_cols)

    if (setequal(keep_vars, actual_fit_vars)) break  # stable

    current_xreg_vars <- keep_vars
    if (length(current_xreg_vars) == 0L) break

    res_refit <- refit_arima(current_xreg_vars)
    if (is.null(res_refit$fit)) break
    arimax_final    <- res_refit$fit
    actual_fit_vars <- res_refit$vars  # update to what model was actually fit on
  }

  sig_vars <- actual_fit_vars  # AUTHORITATIVE: what arimax_final was fit on

  # ---- If all xreg eliminated, refit pure ARIMA (no xreg) ----
  if (length(sig_vars) == 0L) {
    ...
    if (!is.null(pure_arima)) arimax_final <- pure_arima
  }

  # ---- Build xreg test vector from EXACTLY the cols arimax_final knows ----
  # sig_vars == actual_fit_vars == colnames of xreg that was passed to Arima().
  # Any mismatch here causes "Number of regressors does not match fitted model".
