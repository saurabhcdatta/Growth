...
for (sr in names(arima_series_meta)) {
  ...
  for (dt_hm in list(ols_hm_full, ar_hm_full, diff_hm))
    dt_hm[, cat_f := factor(cat_label, levels=cat_order)]
  ...
  hm_path <- file.path(PLOT_DIR,
                        sprintf("%s_heatmap_%s_ols_vs_arima.pdf",
                                hm_pnum, sm$short))
  tryCatch({
    grDevices::cairo_pdf(hm_path, width=16, height=15, onefile=TRUE)
    print(p_hm)
    grDevices::dev.off()
