...
fit_window <- function(train_dt, test_row,
  ...
  refit_arima <- function(vars) {
    # Rebuild xreg from the current active variable set
    xr <- if (length(vars) == 0L) NULL else {
      m    <- X_train[, vars, drop = FALSE]
      ok_v <- apply(m, 2L, function(col) var(col, na.rm = TRUE) > 0)
      m    <- m[, ok_v, drop = FALSE]  # FIX: assign filtered result back
      if (ncol(m) == 0L) NULL else m
    }
    tryCatch(
      forecast::Arima(
        y_ts,
        order          = c(p_ord, d_ord, q_ord),
        seasonal       = if (has_seasonal) c(P_ord, D_ord, Q_ord) else c(0,0,0),
        xreg           = xr,
        include.mean   = (d_ord == 0),   # mean only when not differenced
        include.drift  = (d_ord >= 1)    # drift only when differenced
      ),
      error = function(e) NULL
    )
  }
  ...
  repeat {
    ...
    # Extract xreg coefficients from fitted ARIMAX
    cf_all  <- arimax_final$coef
    cf_se   <- sqrt(diag(arimax_final$var.coef))
    cf_names <- names(cf_all)

    # xreg coefficient names are exactly the column names of xreg matrix
    xreg_coef_names <- intersect(cf_names, current_xreg_vars)
    ...
    # Always enforce sign check (same logic as before)
    if (length(keep_vars) > 0L) {
      keep_vars <- keep_vars[vapply(keep_vars, function(v)
        sign_ok(v, cf_all[v]), logical(1L))]
    }

    # Also keep quarterly dummies regardless of t-stat
    q_dummies <- intersect(c("q1","q2","q3"), current_xreg_vars)
    keep_vars <- unique(c(keep_vars, q_dummies))
    keep_vars <- intersect(keep_vars, x_train_cols)

    if (setequal(keep_vars, current_xreg_vars)) break  # stable

    current_xreg_vars <- keep_vars

    new_fit <- refit_arima(current_xreg_vars)
    if (is.null(new_fit)) break
    arimax_final <- new_fit
  }

  sig_vars <- current_xreg_vars  # final xreg variable set

  # ---- If all xreg eliminated, refit pure ARIMA (no xreg) ----
  # Critical: if arimax_final was fit WITH xreg but we now pass xreg=NULL,
  # forecast::forecast() will throw a dimension-mismatch error and return NA.
  # Fix: when sig_vars is empty, refit as pure ARIMA so xreg=NULL is consistent.
  if (length(sig_vars) == 0L && !is.null(xreg_train)) {
    pure_arima <- tryCatch(
      forecast::Arima(
        y_ts,
        order    = c(p_ord, d_ord, q_ord),
        seasonal = if (has_seasonal) c(P_ord, D_ord, Q_ord) else c(0,0,0),
        include.mean  = (d_ord == 0),
        include.drift = (d_ord >= 1)
      ),
      error = function(e) NULL
    )
  ...
  # ---- Build xreg test vector (only for vars still in model) ----
  build_xreg_test <- function(vars) {
    if (length(vars) == 0L) return(NULL)
    common <- intersect(vars, colnames(X_test_aligned))
    if (length(common) == 0L) return(NULL)
    mat <- X_test_aligned[, common, drop = FALSE]
    if (ncol(mat) == 0L) return(NULL)
    mat
  }
  xreg_test <- if (length(sig_vars) > 0L) build_xreg_test(sig_vars) else NULL
