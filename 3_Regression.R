...
if (nrow(forecasts_all) > 0) {
  sel_over_time <- forecasts_all[!is.na(n_ols_sel),
...
if (nrow(forecasts_all) > 0) {
  method_dt <- forecasts_all[!is.na(method_used),
                              .(n = .N),
...
if (nrow(level_fc) > 0) {
  ...
  for (sr in names(series_meta)) {
    ...
    hist_sr <- hist_long[series == sr]
    fc_sr   <- level_fc[series == sr & !is.na(pred_level)]
  ...
  for (sr in names(series_meta)) {
    ...
    fc_sr <- level_fc[series == sr & !is.na(level_error)]
    if (nrow(fc_sr) == 0) next
  ...
  if (nrow(level_metrics) > 0) {
    p <- ggplot(level_metrics,
                aes(x = reorder(cat_label, r2_oos),
                    y = r2_oos, fill = r2_oos > 0)) +
      geom_col() +
      geom_text(aes(label = sprintf("%.3f", r2_oos),
