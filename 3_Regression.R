...
message(sprintf("    Total candidate features (pre-contemp filter): %d",
                length(FEATS_ALL)))
message("    NOTE: Contemporaneous YoY/QoQ panel transforms are stripped")
message("    per dep_var inside the rolling loop (not in FEATS_ALL).")
message("    Only lagged AR values of dep vars enter the RHS.")
...
if (length(PRE_LASSO_EXCL) > 0L || length(MANUAL_EXCL) > 0L ||
  ...
  message("    [ALWAYS ON] Contemp YoY/QoQ panel transforms excluded per dep_var;")
  message("                AR lags of dep var re-added. Macro exogenous vars kept.")
...
for (dv in names(DEP_VARS)) {
  ...
  for (cat in cats) {
    ...
    for (tq in test_quarters) {
      ...
      # ── Contemporaneous leakage exclusion ────────────────────
      # Rule: the RHS must contain ONLY lagged (autoregressive) values
      # of endogenous panel variables.  Contemporaneous YoY and QoQ
      # transforms are realised at the same time as the dependent
      # variable and would introduce spurious contemporaneous correlation.
      #
      # What we strip from feats_use for ALL dep_vars:
      #   1. ^yoy_[var]   where var is NOT a purely exogenous macro series
      #   2. ^qoq_[var]   same rule
      #   3. _accel$      acceleration (diff of YoY) — same-period
      #   4. _cyc$        cyclical deviation — same-period
      #
      # What we KEEP (safe):
      #   • *_lag[N]* forms of any variable — already lagged, no leakage
      #   • Macro-level contemporaneous series:
      #       fedfunds, gs2, gs10, yield_curve*, spread*, unrate,
      #       cpi_yoy, real_rate, mortgage30, hy_spread, baa_spread,
      #       credit_tightness, oil_*, fomc_regime, hike_run
      #       (these are EXOGENOUS to credit union outcomes)
      #   • Regime dummies, time_idx, q1/q2/q3, share_, merger/exit rates
      #
      # Additionally: strip anything that directly encodes THIS dep_var
      # (strict target-leakage guard, same as before).

      feats_use <- FEATS_ALL

      # ── Exogenous macro series — safe to use contemporaneously ──
      # Matches column names that are purely macro / external drivers.
      MACRO_CONTEMP_PAT <- paste(
        "^fedfunds($|_)",
        "^gs2$", "^gs10$",
        "^yield_curve",
        "^spread_",
        "^hy_spread$", "^baa_spread$",
        "^credit_tightness$",
        "^unrate$",
        "^cpi_yoy$",           # macro CPI — exogenous
        "^real_rate$",
        "^mortgage30$",
        "^oil_",
        "^fomc_regime$", "^hike_run$",
      ...
      # ── Strip contemporaneous YoY / QoQ of panel (endogenous) vars ──
      # A feature is "contemporaneous panel" if:
      #   (a) it starts with yoy_ or qoq_, OR ends with _accel / _cyc
      #   (b) it does NOT already contain _lag (i.e. it is NOT lagged)
      #   (c) it does NOT match the exogenous macro pattern above
      is_contemp_panel <- function(v) {
        is_transform <- grepl("^yoy_|^qoq_|_accel$|_cyc$", v, perl=TRUE)
        is_lagged    <- grepl("_lag[0-9]", v, perl=TRUE)
        is_macro     <- grepl(MACRO_CONTEMP_PAT, v, perl=TRUE)
        is_transform && !is_lagged && !is_macro
      }

      contemp_panel_vars <- feats_use[vapply(feats_use, is_contemp_panel,
                                              logical(1))]

      if (length(contemp_panel_vars) > 0L) {
        feats_use <- setdiff(feats_use, contemp_panel_vars)
        # Only log on the FIRST rolling window to avoid console flood
        if (tq == test_quarters[1L]) {
          message(sprintf(
            "        [CONTEMP EXCL] %s — removed %d contemp panel transforms: %s",
            dv, length(contemp_panel_vars),
            paste(head(sort(contemp_panel_vars), 8), collapse=", "),
            if (length(contemp_panel_vars) > 8) "..." else ""
          ))
        }
      }

      # ── Target-leakage guard — strip anything encoding THIS dep_var ──
      if (dv == "yoy_ficu_pct") {
        # Remove ALL ficu-related transforms (contemporaneous and lagged
        # of the TARGET itself — AR lags are re-added explicitly below)
        feats_use <- feats_use[!grepl("^yoy_ficu|^qoq_ficu", feats_use, perl=TRUE)]
      }
      if (dv == "yoy_fiscu_pct") {
        feats_use <- feats_use[!grepl("^yoy_fiscu|^qoq_fiscu", feats_use, perl=TRUE)]
      }
      if (dv == "ln_assets_tot") {
        # Remove all asset-level and asset-transform columns
        feats_use <- feats_use[!grepl("assets", feats_use, perl=TRUE)]
      }

      # ── Re-add AR (autoregressive) lags of the dependent variable ──
      # These are the lagged values of the target — legitimate predictors.
      # Pattern: <base>_lag[N]  where base matches the dep_var family.
      ar_pattern <- switch(dv,
        "yoy_ficu_pct"  = "^yoy_ficu.*_lag[0-9]",
        "yoy_fiscu_pct" = "^yoy_fiscu.*_lag[0-9]",
        "ln_assets_tot" = "^ln_assets.*_lag[0-9]",
        NULL
      )
      if (!is.null(ar_pattern)) {
        ar_lags <- grep(ar_pattern, names(train_dt), value=TRUE, perl=TRUE)
        ar_lags <- ar_lags[vapply(ar_lags,
          function(cn) is.numeric(train_dt[[cn]]), logical(1))]
        if (length(ar_lags) > 0L) {
          feats_use <- unique(c(feats_use, ar_lags))
          if (tq == test_quarters[1L])
            message(sprintf("        [AR LAGS] %s adding: %s",
                            dv, paste(ar_lags, collapse=", ")))
        }
      }
