...
#
# dep_var  : one of "yoy_ficu_pct", "yoy_fiscu_pct", "ln_assets_tot"
...
#
#   Remove baa_spread from ln_assets_tot in the >$10B bucket only:
...
#   dep_var   : "yoy_ficu_pct" | "yoy_fiscu_pct" | "ln_assets_tot"
#               or "*" for all three
#   cat       : exact cat_label  e.g. "3_50M_100M"
...
#     Only sensible for YoY% dep_vars; ignored for ln_assets_tot.
...
#   list(
#     dep_var  = "<dep_var>",       # "yoy_ficu_pct" | "yoy_fiscu_pct" | "ln_assets_tot"
  ...
  if (length(selected) >= 1L) {
    ...
    if (length(ols_feats) >= 1L) {
      ...
      if (!is.null(ols_fit)) {
        ...
        # Re-fit OLS keeping only significant + sign-ok variables.
        # Repeat until stable: removing vars from one pass changes
        # p-values in the next, so iterate until no more are dropped.
        # Quarterly dummies (q1/q2/q3) are always protected from removal.
        MAX_ITER <- 10L
        iter     <- 0L
        ols_fit2 <- NULL

        if (length(sig_vars) >= 1L) {
          current_vars <- sig_vars   # starts with LASSO-selected sig vars
          repeat {
            iter <- iter + 1L
            if (length(current_vars) == 0L || iter > MAX_ITER) break

            ols_dt2    <- as.data.frame(X_train[, current_vars, drop=FALSE])
            ols_dt2$y__ <- y_train_w
            fit_iter   <- tryCatch(lm(y__ ~ ., data=ols_dt2),
                                   error=function(e) NULL)
            if (is.null(fit_iter)) break

            sm2   <- summary(fit_iter)
            cf2   <- as.data.frame(sm2$coefficients)
            cf2$var <- rownames(cf2)
            cf2   <- cf2[cf2$var != "(Intercept)", , drop=FALSE]
            names(cf2)[4] <- "pval"

            cf2$sign_ok <- mapply(sign_ok, cf2$var, cf2$Estimate)

            # Variables to keep: (significant AND sign-ok) OR quarterly dummy
            q_protected <- intersect(c("q1","q2","q3"), current_vars)
            keep_vars   <- union(
              cf2$var[cf2$pval < sig_level & cf2$sign_ok],
              q_protected
            )

            if (setequal(keep_vars, current_vars)) {
              # Stable — no more variables to drop
              ols_fit2 <- fit_iter
              break
            }
            if (length(keep_vars) == 0L) {
              # All non-dummy vars dropped; keep only dummies if any
              if (length(q_protected) > 0L) {
                ols_dt2b    <- as.data.frame(X_train[, q_protected, drop=FALSE])
                ols_dt2b$y__ <- y_train_w
                ols_fit2    <- tryCatch(lm(y__ ~ ., data=ols_dt2b),
                                        error=function(e) NULL)
              }
              break
            }
            current_vars <- keep_vars   # loop with reduced set
          }  # end repeat

          # Update sig_vars to the final stable set
          if (!is.null(ols_fit2)) {
            final_cf  <- as.data.frame(summary(ols_fit2)$coefficients)
            final_cf$var <- rownames(final_cf)
            final_cf  <- final_cf[final_cf$var != "(Intercept)", , drop=FALSE]
            sig_vars  <- final_cf$var
          }
...
for (dv in names(DEP_VARS)) {
  ...
  for (cat in cats) {
    ...
    for (tq in test_quarters) {
      ...
      if (dv == "ln_assets_tot") {
        # Strip contemporaneous assets transforms (the target family)
        # Keep asset lags — they are valid AR predictors
        assets_contemp <- grep("^(yoy|qoq)_assets|^ln_assets_tot$|assets.*(accel|cyc)$",
                               feats_use, perl=TRUE, value=TRUE)
        # Exclude anything that is already a lag (safe to keep)
      ...
      # Shows full OLS summary for every one of the 21 models so the
      # analyst can inspect coefficients, p-values, and R² at a glance.
      if (tq == tail(test_quarters, 1L) && res$ok && !is.null(res$ols_fit2)) {
...
",
        ...
        if (is_ov) {
          cat(sprintf("  Override vars (%d): %s
%s
",
                      length(res$sig_vars),
                      paste(res$sig_vars, collapse=", "),
                      strrep("─", 72)))
        }
        print(summary(res$ols_fit2))
        cat(sprintf(
          "  LASSO selected: %d   OLS final: %d   Method: %s
%s
",
          res$n_lasso_sel, length(res$sig_vars), res$method_used,
          strrep(if (is_ov) "▶" else "═", 72)
        ))
      }
        ...
        for (i in idx) {
          ...
          new_pred <- switch(ov$type,
            ...
            "growth_rate" = {
              ...
              if (dv_k == "ln_assets_tot") {
                # ln_assets quarterly increment ≈ log(1 + rate/400)
                # (compound quarterly from annualised %)
                anchor_ln <- forecasts_all$anchor_lnassets[i]
                if (!is.na(anchor_ln)) {
                  anchor_ln + log(1 + as.numeric(ov$value) / 400)
                } else {
                  orig_pred   # can't apply without anchor
                }
              } else {
                # YoY% model: growth_rate IS the predicted YoY%
                as.numeric(ov$value)
              }
            },
            ...
            "flat" = {
              ...
              if (dv_k == "ln_assets_tot") {
                # Keep ln level unchanged: find last actual ln_assets
                last_actual <- forecasts_all[
                  dep_var   == dv_k &
                  cat_label == cat_k &
                  !is.na(actual) &
                  date < date_i
                ][order(-date)][1L, actual]
                if (!is.na(last_actual)) last_actual else orig_pred
              } else {
                0   # YoY% = 0 → no growth
              }
            },
...
#
# For every series × category (21 combinations per series = 63
# total) we:
#   1. Fit auto.arima on the full historical level series
#   2. Forecast 20 quarters ahead (5 years) from TRAIN_END
#   3. Mark 1-yr (4Q), 3-yr (12Q), 5-yr (20Q) horizon points
#
# Outputs:
#   P19_forecast_pdf/  — one PDF per dep_var (7 panels each),
#                        showing Actual | OLS | ARIMA + horizons
#   P20_heatmap_ficu_vs_arima.pdf   — OLS vs ARIMA MAPE heatmap
#   P21_heatmap_fiscu_vs_arima.pdf
#   P22_heatmap_assets_vs_arima.pdf
# ════════════════════════════════════════════════════════════
message("
[6d] ARIMA baselines + forecast PDF + heatmaps...")

# ── Ensure forecast package is available ─────────────────────
if (!requireNamespace("forecast", quietly=TRUE)) {
  message("    Installing forecast package...")
  install.packages("forecast", repos="https://cloud.r-project.org", quiet=TRUE)
}
suppressPackageStartupMessages(library(forecast))

# ── Output sub-folder for individual model PDFs ───────────────
fc_pdf_dir <- file.path(PLOT_DIR, "P19_forecast_pdf")
if (!dir.exists(fc_pdf_dir)) dir.create(fc_pdf_dir, recursive=TRUE)

# ── ARIMA horizon config ──────────────────────────────────────
HORIZON_QTR <- 20L          # 5-year horizon in quarters
HORIZON_MARKS <- c(4L, 12L, 20L)  # 1yr, 3yr, 5yr
HORIZON_LABS  <- c("1-yr", "3-yr", "5-yr")

# ── Series metadata for level plots ──────────────────────────
arima_series_meta <- list(
  ficu_count  = list(
    label      = "FICU Count",
    hist_col   = "ficu_count",
    level_col  = "ficu_count",
    fmt        = "comma",
    dep_var    = "yoy_ficu_pct",
    short      = "ficu"
  ),
  fiscu_count = list(
    label      = "FISCU Count",
    hist_col   = "fiscu_count",
    level_col  = "fiscu_count",
    fmt        = "comma",
  ...
  total_assets = list(
    ...
    hist_col   = "assets_tot",
    level_col  = "assets_tot",
    fmt        = "dollar",
    dep_var    = "ln_assets_tot",
    short      = "assets"
  )
)

# ── Fit ARIMA and collect forecasts for all 21 series×cats ───
message("    Fitting ARIMA baselines (21 series per metric = 63 models)...")
arima_fc_list <- list()
...
for (sr in names(arima_series_meta)) {
  ...
  for (cat in cats) {
    ...
    cat_hist <- full_levels[cat_label == cat]
    setorderv(cat_hist, "date")

    # Use data up to TRAIN_END as training for ARIMA (comparable to OLS)
    train_hist <- cat_hist[date <= TRAIN_END]
    y_ts <- ts(train_hist[[hcol]], frequency=4)

    if (length(y_ts) < 12L || all(is.na(y_ts))) {
      message(sprintf("    [ARIMA SKIP] %s|%s — insufficient history", sr, cat))
      next
    }

    # Fit auto.arima with stepwise for speed
    arima_fit <- tryCatch(
    ...
    if (is.null(arima_fit)) {
      message(sprintf("    [ARIMA FAIL] %s|%s: %s", sr, cat, conditionMessage(e)))
      next
    }

    # Forecast HORIZON_QTR quarters ahead
    arima_pred <- tryCatch(
    ...
    # Build date sequence for forecast quarters
    last_train_date <- max(train_hist$date)
    fc_dates_arima  <- last_train_date + seq_len(HORIZON_QTR) / 4
    ...
    message(sprintf("    [ARIMA] %s|%s: %s  h=%d",
                    sr, cat,
                    arima_fc_list[[key]]$arima_order[1],
                    HORIZON_QTR))
  }
}
...
message(sprintf("    ARIMA forecasts: %d rows across %d models",
                nrow(arima_fc_all), length(arima_fc_list)))

# ── Helper: format y-axis by series ──────────────────────────
fmt_y_arima <- function(sr) {
  if (sr == "total_assets")
    scale_y_continuous(labels=scales::dollar_format(scale=1e-6, suffix="M"))
  else
    scale_y_continuous(labels=scales::comma)
...
# ════════════════════════════════════════════════════════════
# P19: 21-model forecast PDF (one page per dep_var, 7 panels)
#      Each panel: historical + OLS forecast + ARIMA forecast
#      Vertical dashed lines at 1/3/5-yr horizon marks
# ════════════════════════════════════════════════════════════
message("
    P19: 21-model forecast panels PDF...")

if (nrow(arima_fc_all) > 0 && nrow(level_fc) > 0) {
  ...
  for (sr in names(arima_series_meta)) {
    ...
    # Historical actuals
    hist_dt <- full_levels[, .(date, cat_label, actual=get(hcol))]
    hist_dt[, date_d := as.Date(date)]

    # OLS level forecast for this series
    ols_dt <- level_fc[series == sr,
                       .(cat_label, date, ols_point=pred_level,
                         ols_lo=pred_lo95, ols_hi=pred_hi95)]
    ols_dt[, date_d := as.Date(date)]

    # ARIMA forecast for this series
    ar_dt <- arima_fc_all[series == sr,
                           .(cat_label, date, arima_point,
                             arima_lo95, arima_hi95, arima_order)]
    ar_dt[, date_d := as.Date(date)]

    if (nrow(ols_dt)==0 && nrow(ar_dt)==0) next

    # Horizon marker dates (from TRAIN_END)
    train_end_d    <- as.Date(TRAIN_END)
    horizon_dates  <- train_end_d + HORIZON_MARKS * (365.25/4)

    # Build plot per category, collect into list then wrap in one PDF
    panel_plots <- list()

    for (cat in cats) {
      h_cat  <- hist_dt[cat_label==cat]
      o_cat  <- ols_dt[cat_label==cat]
      a_cat  <- ar_dt[cat_label==cat]

      if (nrow(h_cat)==0) next

      # Get ARIMA model string for subtitle
      arima_str <- if (nrow(a_cat)>0) a_cat$arima_order[1] else "N/A"

      p <- ggplot() +
        # Amber shading for forecast period
        annotate("rect",
                 xmin = train_end_d, xmax = Inf,
                 ymin = -Inf,        ymax = Inf,
                 fill = "#fff3cd",   alpha = 0.5) +
        # Historical actual
        geom_line(data=h_cat,
                  ...
                  linewidth=0.9) +
        # OLS CI ribbon
        {if (nrow(o_cat)>0 && any(!is.na(o_cat$ols_lo)))
           geom_ribbon(data=o_cat,
                       aes(x=date_d, ymin=ols_lo, ymax=ols_hi),
                       fill="#2171b5", alpha=0.15)
         else list()} +
        # OLS point forecast
        {if (nrow(o_cat)>0)
           geom_line(data=o_cat,
                     aes(x=date_d, y=ols_point, colour="OLS Forecast"),
                     linewidth=0.85, linetype="dashed")
         else list()} +
        # ARIMA CI ribbon
        {if (nrow(a_cat)>0)
        ...
        {if (nrow(a_cat)>0)
           geom_line(data=a_cat,
                     aes(x=date_d, y=arima_point, colour="ARIMA Baseline"),
                     linewidth=0.85, linetype="dotdash")
         else list()} +
        # Vertical line at forecast start
        geom_vline(xintercept=train_end_d,
                   linetype="dotted", colour="grey40", linewidth=0.5) +
        # Horizon markers (1yr / 3yr / 5yr)
        geom_vline(xintercept=horizon_dates,
                   linetype="longdash", colour="#7f7f7f", linewidth=0.4) +
        annotate("text",
        ...
        scale_colour_manual(
          values=c("Actual"="#1f77b4",
                   "OLS Forecast"="#d62728",
                   "ARIMA Baseline"="#2ca02c"),
          name=NULL) +
        fmt_y_arima(sr) +
        scale_x_date(date_labels="%Y", date_breaks="2 years") +
        labs(
          title    = cat,
          subtitle = sprintf("ARIMA baseline: %s  |  95%% CI shaded", arima_str),
          x=NULL, y=label
        ) +
        theme_cu() +
    ...
    # Combine all category panels into one PDF page using patchwork
    ...
    combined <- patchwork::wrap_plots(panel_plots, ncol=3) +
      patchwork::plot_annotation(
        title    = sprintf("Forecast Model: %s", label),
        subtitle = sprintf(
          "Post-LASSO OLS (dashed/blue) vs ARIMA Baseline (dot-dash/green) | Amber = OOS | Horizons: 1yr / 3yr / 5yr
Training ends: %s",
          as.character(TRAIN_END)),
        theme = theme(
          plot.title    = element_text(face="bold", size=14),
    ...
    tryCatch({
      ...
      grDevices::dev.off()
      message(sprintf("    [WARN] P19 PDF failed for %s: %s", sr, e$message))
    })
  }
}

# ════════════════════════════════════════════════════════════
# P20-P22: HEATMAPS — OLS vs ARIMA comparison
#
# For each series (ficu / fiscu / assets) produce one heatmap
# PDF with two side-by-side panels:
#   Left  : OLS     MAPE by (quarter × category)
#   Right : ARIMA   MAPE by (quarter × category)
# Colour: white = 0% error, dark red = high error.
# Additional strip below: OLS − ARIMA MAPE (positive = OLS worse)
# ════════════════════════════════════════════════════════════
message("
    P20-P22: OLS vs ARIMA heatmaps...")
...
for (sr in names(arima_series_meta)) {
  ...
  # ── OLS errors ───────────────────────────────────────────
  ols_err <- level_fc[series==sr & !is.na(actual_level) & !is.na(pred_level),
                       .(cat_label, date,
                         ols_mape = abs((actual_level - pred_level) /
                                          actual_level) * 100)]

  # ── ARIMA errors: match to actual levels from full_levels ──
  ar_sr <- arima_fc_all[series==sr]
  hcol  <- sm$hist_col
  act_dt <- full_levels[, .(date, cat_label, actual_level=get(hcol))]

  ar_err <- merge(ar_sr, act_dt, by=c("cat_label","date"), all.x=TRUE)
  ar_err <- ar_err[!is.na(actual_level) & !is.na(arima_point) & actual_level!=0,
                    .(cat_label, date,
                      arima_mape = abs((actual_level - arima_point) /
                                         actual_level) * 100)]

  if (nrow(ols_err)==0 && nrow(ar_err)==0) {
    message(sprintf("    [HEATMAP SKIP] %s — no error data", sr))
    next
  }

  # ── Merge OLS + ARIMA errors ──────────────────────────────
  err_both <- merge(ols_err, ar_err, by=c("cat_label","date"), all=TRUE)
  err_both[, date_d   := as.Date(date)]
  err_both[, yr_qtr   := format(date_d, "%Y Q%q")]  # label for x-axis
  err_both[, diff_mape := ols_mape - arima_mape]     # positive = OLS worse

  # Cap MAPE at 20% for colour scale readability
  MAPE_CAP <- 20
  err_both[, ols_mape_cap   := pmin(ols_mape,   MAPE_CAP, na.rm=TRUE)]
  err_both[, arima_mape_cap := pmin(arima_mape, MAPE_CAP, na.rm=TRUE)]
  err_both[, diff_cap       := pmax(pmin(diff_mape, MAPE_CAP, na.rm=TRUE),
                                     -MAPE_CAP, na.rm=TRUE)]

  # Factor for y-axis ordering (largest category at top)
  cat_order <- rev(sort(unique(err_both$cat_label)))
  err_both[, cat_f := factor(cat_label, levels=cat_order)]

  # ── Heatmap helper ──────────────────────────────────────
  make_heatmap_panel <- function(dt, fill_var, fill_label,
                                  title_txt, low_col, high_col,
                                  mid_col=NA, midpoint=NA) {
    aes_fill <- if (!is.na(midpoint))
      ...
      scale_fill_gradient(low=low_col, high=high_col,
                          na.value="grey90", name=fill_label,
                          labels=function(x) sprintf("%.1f%%", x))

    ggplot(dt, aes(x=date_d, y=cat_f,
                   fill=get(fill_var))) +
  ...
  # Panel A: OLS MAPE
  p_ols <- make_heatmap_panel(
    dt         = err_both,
    fill_var   = "ols_mape_cap",
    fill_label = sprintf("MAPE %%
(capped at %d%%)", MAPE_CAP),
    title_txt  = sprintf("OLS Forecast MAPE — %s", label),
    low_col    = "#f7fbff",
    high_col   = "#08306b"
  )

  # Panel B: ARIMA MAPE
  p_arima <- make_heatmap_panel(
    dt         = err_both,
    fill_var   = "arima_mape_cap",
    fill_label = sprintf("MAPE %%
(capped at %d%%)", MAPE_CAP),
    title_txt  = sprintf("ARIMA Baseline MAPE — %s", label),
    low_col    = "#fff5f0",
    high_col   = "#67000d"
  )

  # Panel C: Difference (OLS − ARIMA MAPE)
  # Blue = OLS better (OLS MAPE lower), Red = ARIMA better
  p_diff <- make_heatmap_panel(
    dt         = err_both,
    fill_var   = "diff_cap",
    fill_label = "OLS − ARIMA
MAPE (pp)",
    title_txt  = "OLS vs ARIMA: MAPE Difference (positive = OLS worse)",
    low_col    = "#2166ac",   # blue = OLS better
    high_col   = "#d6604d",   # red  = ARIMA better
    mid_col    = "#f7f7f7",
    midpoint   = 0
  )
  ...
  combined_hm <- (p_ols / p_arima / p_diff) +
    patchwork::plot_annotation(
      title    = sprintf("%s — OLS vs ARIMA Forecast Accuracy Heatmap", label),
      subtitle = paste(
        "MAPE by quarter × asset category.",
        sprintf("Top: OLS (blue scale).  Middle: ARIMA (red scale)."),
        "Bottom: difference (blue = OLS better, red = ARIMA better).",
        sprintf("Capped at %d%% for colour scale.", MAPE_CAP)
      ),
      theme = theme(
        plot.title    = element_text(face="bold", size=13),
  ...
  hm_path <- file.path(PLOT_DIR,
                        sprintf("%s_heatmap_%s_ols_vs_arima.pdf",
                                hm_pnum, sm$short))
  tryCatch({
    grDevices::cairo_pdf(hm_path, width=16, height=14, onefile=TRUE)
    print(combined_hm)
    grDevices::dev.off()
    message(sprintf("    Saved: %s", hm_path))
  }, error=function(e) {
    grDevices::dev.off()
