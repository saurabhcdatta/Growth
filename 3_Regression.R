...
fit_window <- function(train_dt, test_row,
  ...
  is_arima000 <- all(c(p_ord, d_ord, q_ord, P_ord, D_ord, Q_ord) == 0)
  ...
  r2_tmp      <- if (ss_tot_tmp > 0) 1 - ss_res_tmp / ss_tot_tmp else NA_real_
  adj_r2_tmp  <- if (!is.na(r2_tmp) && n_tmp > k_tmp + 1L)
                   1 - (1 - r2_tmp) * (n_tmp - 1L) / (n_tmp - k_tmp - 1L)
                 else NA_real_

  if (is_arima000 && !is.na(adj_r2_tmp) && adj_r2_tmp > OVERFIT_ADJ_R2_MAX &&
      length(sig_vars) > 2L) {
    message(sprintf(
