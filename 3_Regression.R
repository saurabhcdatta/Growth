  ...
  for (cat in cats) {
    ...
    for (tq in test_quarters) {
      ...
      FICU_FISCU_CONTEMP_PAT <-
        "^(yoy|qoq)_(ficu|fiscu)(?!.*_lag)|_(ficu|fiscu).*(_accel|_cyc)$"

      if (dv == "yoy_ficu_pct") {
        # Strip all contemporaneous ficu transforms (the target itself)
        # and contemporaneous fiscu transforms (same-period cross-series)
        # Lags of both are kept — they are valid predictors
        contemp_drop <- grep(FICU_FISCU_CONTEMP_PAT, feats_use,
                             perl=TRUE, value=TRUE)
        # Also strip direct same-period yoy_ficu_pct (the exact target)
        contemp_drop <- union(contemp_drop,
                              grep("^yoy_ficu_pct$", feats_use, value=TRUE))
        feats_use <- setdiff(feats_use, contemp_drop)
        if (tq == test_quarters[1L] && length(contemp_drop) > 0L)
          message(sprintf("        [FICU CONTEMP EXCL] removed %d: %s",
                          length(contemp_drop),
      ...
      if (dv == "yoy_fiscu_pct") {
        contemp_drop <- grep(FICU_FISCU_CONTEMP_PAT, feats_use,
                             perl=TRUE, value=TRUE)
        contemp_drop <- union(contemp_drop,
                              grep("^yoy_fiscu_pct$", feats_use, value=TRUE))
        feats_use <- setdiff(feats_use, contemp_drop)
        if (tq == test_quarters[1L] && length(contemp_drop) > 0L)
          message(sprintf("        [FISCU CONTEMP EXCL] removed %d: %s",
                          length(contemp_drop),
                          paste(sort(contemp_drop), collapse=", ")))
      }
