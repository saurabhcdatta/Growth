...
#  - Directional accuracy & Diebold-Mariano test vs naive
#  - Skill score (RMSE relative to naive)
#  - Pooled model (all categories) as comparison
#  - Full execution timing (hours, minutes, seconds)
#  - ntfy.sh push notifications at key milestones
#
# SETUP (one-time, takes 2 minutes):
#  1. Install the free "ntfy" app on your phone (iOS or Android)
#  2. Open the app and subscribe to a topic — pick any unique
#     name e.g. "cu-forecast-2024" (keep it private)
#  3. Set NTFY_TOPIC below to that same topic name
#  4. That's it — no account, no API key needed
############################################################
...
suppressPackageStartupMessages({
  ...
  library(doParallel)
  library(ggplot2)
  library(httr)        # for ntfy HTTP calls
})
set.seed(42)

# ══════════════════════════════════════════════════════════
# ntfy CONFIGURATION
# Change NTFY_TOPIC to the topic you subscribed to in the app
# ══════════════════════════════════════════════════════════
NTFY_TOPIC   <- "RCode"   # <-- CHANGE THIS
NTFY_ENABLED <- TRUE                        # set FALSE to silence all notifications

# ntfy notification function
# priority: "default", "low", "high", "urgent"
# tags:     emoji shortcodes e.g. "white_check_mark", "warning", "x"
notify <- function(title, message, priority = "default", tags = NULL) {
  ...
  tryCatch({
    ...
    if (!is.null(tags))
      headers <- c(headers, Tags = paste(tags, collapse = ","))

    httr::POST(
      url     = paste0("https://ntfy.sh/", NTFY_TOPIC),
      body    = message,
      encode  = "raw",
    ...
    message(sprintf("  [ntfy] Could not send notification: %s", e$message))
    invisible(NULL)
  })
}
...
SCRIPT_START_TIME <- format(Sys.time(), "%Y-%m-%d %H:%M:%S")

message(strrep("=", 55))
message("PART 2 — XGBoost CU Forecast Pipeline")
message(sprintf("Started: %s", SCRIPT_START_TIME))
message(strrep("=", 55))

# Kick off notification
notify(
  title    = "CU Forecast Started",
  message  = sprintf("Part 2 XGBoost pipeline started at %s", SCRIPT_START_TIME),
  priority = "default",
  tags     = "rocket"
)
...
# ── CATEGORY DIAGNOSTIC ───────────────────────────────────
# Prints class and unique values so you can confirm real labels
message(sprintf("  categories column class: %s", class(qtrly[[GROUP_COL]])))
message("  Raw unique values:")
print(sort(unique(qtrly[[GROUP_COL]])))

# Force character — guarantees list names = real labels, not factor integers
...
# Base R split — always names by actual value, never by factor integer
cat_list <- split(qtrly, f = qtrly[[GROUP_COL]])
cats     <- sort(names(cat_list))

message(sprintf("  cat_list keys: %s", paste(cats, collapse = " | ")))
...
run_category <- function(dt_cat, cat_value) {
  ...
  # Ensure character GROUP_COL in worker copy
  ...
  if (is.null(dt_cat) || nrow(dt_cat) == 0)
    return(list(category = cat_value,
                error = "Empty data slice received by worker"))
  ...
  # Fallback if every CV iteration failed
...
notify(
  ...
  message = sprintf("Running %d categories on %d cores. %d search iters per category.",
                    length(cats), N_CORES, N_SEARCH_ITER),
...
n_ok     <- sum(vapply(results, function(r) is.null(r$error) || is.na(r$error), logical(1)))
...
# Notify: grid search done with pass/fail summary
if (n_failed == 0) {
  notify(
    title    = "Grid Search Done",
    message  = sprintf("All %d categories completed OK in %s",
                       n_ok, fmt_elapsed(parallel_time)),
    priority = "default",
    tags     = "white_check_mark"
  )
} else {
  failed_names <- paste(
    vapply(results[vapply(results, function(r) !is.null(r$error) && !is.na(r$error), logical(1))],
           function(r) r$category, character(1)),
    collapse = ", "
  )
  notify(
    title    = "Grid Search Done (with failures)",
    message  = sprintf("%d/%d OK in %s. FAILED: %s",
                       n_ok, length(results),
                       fmt_elapsed(parallel_time), failed_names),
    priority = "high",
    tags     = "warning"
  )
}
...
plot_actual_vs_pred <- function(cat_value) {
  ...
  ggplot(df, aes(x = date_d)) +
    ...
    labs(title    = paste0("YoY FICU Count | ", cat_value),
         subtitle = paste0("Shaded = test period  |  ", skill_lbl, "  |  ", dir_lbl),
...
plot_top10_importance <- function(cat_value) {
  ...
  ggplot(df, aes(x = Feature, y = Gain, fill = feat_type)) +
    ...
    scale_fill_manual(values = c("Lag / Rolling" = "#2171b5",
                                 "Trend / Cycle" = "#238b45",
                                 "Macro"         = "#d94801",
                                 "Internal"      = "#756bb1")) +
...
# 14e) Execution time per category
plot_timing <- function() {
  ...
  ggplot(dt, aes(x = reorder(categories, total_seconds),
    ...
    labs(title = "Execution Time per Category", x = NULL, y = "Minutes") +
    theme_bw(base_size = 11)
}
...
# ── 16) Total script runtime & final notification ─────────
...
# Build a results summary for the final notification
if (n_ok > 0 && "skill_score" %in% names(metrics_dt)) {
  best_cat   <- metrics_dt[!is.na(skill_score)][which.max(skill_score), categories]
  best_skill <- metrics_dt[!is.na(skill_score)][which.max(skill_score), skill_score]
  pool_line  <- if (!is.null(pooled_res))
    sprintf("\nPooled RMSE: %.4f", pooled_res$test_rmse) else ""

  notify(
    title    = "CU Forecast Complete!",
    message  = sprintf(
      "%d/%d categories OK | Runtime: %02dh %02dm\nBest: %s (skill %.1f%%)%s\nResults saved to xgb_metrics_enriched.csv",
      n_ok, length(results),
      total_hrs, total_mins,
      best_cat, best_skill,
      pool_line
    ),
    priority = if (n_failed > 0) "high" else "default",
    tags     = if (n_failed == 0) "tada" else "warning"
  )
} else {
  notify(
    title    = "CU Forecast Complete",
    message  = sprintf("Runtime: %02dh %02dm %04.1fs | %d/%d categories OK",
                       total_hrs, total_mins, total_s,
                       n_ok, length(results)),
    priority = if (n_failed > 0) "high" else "default",
    tags     = if (n_failed == 0) "white_check_mark" else "warning"
  )
}
