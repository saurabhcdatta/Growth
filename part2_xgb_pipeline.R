...
# Filter date range
qtrly <- qtrly[date >= zoo::as.yearqtr("2005 Q1")]
setorderv(qtrly, c("cat_label","date"))
...
# Attach macro/regime features using column assignment (no merge needed)
# These are identical across categories — grab from first category by date
macro_cols <- intersect(FEATS, names(qtrly))
macro_cols <- macro_cols[!startsWith(macro_cols, "yoy_") &
                          !startsWith(macro_cols, "qoq_")]
macro_cols <- setdiff(macro_cols, names(system_dt))  # skip already-present

if (length(macro_cols) > 0) {
  # One row per date from first category — same macro values for all cats
  cat1_ref <- qtrly[cat_label == cats[1],
                     c("date", macro_cols), with = FALSE]
  cat1_ref  <- unique(cat1_ref, by = "date")
  setorderv(cat1_ref, "date")

  # Match rows by date and assign columns directly — no merge()
  idx <- match(system_dt$date, cat1_ref$date)
  for (col in macro_cols) {
    system_dt[, (col) := cat1_ref[[col]][idx]]
  }
  message(sprintf("    Attached %d macro cols to system_dt", length(macro_cols)))
}

# Features available in system_dt for the overall model
...
message(sprintf("    Overall model features: %d", length(sys_feats_avail)))
