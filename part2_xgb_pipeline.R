...
# Remove 'date' and anything already in system_dt to prevent duplicate cols
macro_cols <- setdiff(macro_cols, c("date", names(system_dt)))
if (length(macro_cols) > 0) {
  cat1_macro <- unique(qtrly[cat_label == cats[1],
                              c("date", macro_cols), with = FALSE])
  # Final safety: drop any cols already in system_dt before merging
  drop_cols  <- intersect(names(cat1_macro), names(system_dt))
  drop_cols  <- setdiff(drop_cols, "date")   # keep date for join key
  if (length(drop_cols) > 0)
    cat1_macro[, (drop_cols) := NULL]
